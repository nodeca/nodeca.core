...
N.wire.on('io.error', function (err) {
  if (N.io.INVALID_CSRF_TOKEN === err.code) {
    ...
    N.wire.emit('lib.notification', t('invalid_csrf_token'));

  } else if (N.io.APP_ERROR === err.code) {
    N.wire.emit('lib.notification', t('application_fuckup'));

  } else if (N.io.ECOMMUNICATION === err.code) {
    N.wire.emit('lib.notification', t('communication_timeout'));
  }
});
