...
let view = null;
let SELECTOR = '#rebuild-image-sizes-fetch-task';
let finished_tasks = {};
...
N.wire.on('navigate.done:admin.core.rebuild', function rebuild_image_sizes_fetch_widget_setup() {
  ...
  let current = N.runtime.page_data.image_sizes_fetch_task.current || 0;
  let total   = N.runtime.page_data.image_sizes_fetch_task.total || 1;
...
N.wire.once('navigate.done:admin.core.rebuild', function rebuild_image_sizes_fetch_widget_setup_handlers() {
  ...
  N.wire.on(module.apiPath + '.start', function rebuild_start() {
    N.io.rpc('admin.core.rebuild.image_sizes_fetch.start').then(() => {
  ...
  N.wire.on(module.apiPath + '.stop', function rebuild_stop() {
    N.io.rpc('admin.core.rebuild.image_sizes_fetch.stop').then(() => {
      // reset progress bar to zero
      view.current(0);
      view.total(1);
      view.started(false);
