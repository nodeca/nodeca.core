...
module.exports = function (N) {
  ...
  N.wire.before('server_chain:*', { priority: -90 }, function cookies_start(env, callback) {
    ...
    // Helper to change value or options of already created outgoing cookie.
    function updateCookie(name, options) {
      options = options || {};

      var cookie = setCookie.storage[name];

      if (!cookie) {
        N.logger.error('Unable to update non-existent outgoing cookie %j', name);
        return;
      }

      cookie.value = options.value || cookie.value;
      _.extend(cookie.options, _.omit(options, 'value'));
    }
    ...
    env.extras.updateCookie = updateCookie;
    callback();
  });
