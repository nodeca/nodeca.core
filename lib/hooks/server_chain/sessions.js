...
module.exports = function (N) {
  ...
  N.wire.after('server_chain:*', { priority: 90, ensure: true }, function session_end(env) {
    ...
    env.extras.setCookie('sid', req.sid, {
      maxAge: SESSION_TTL
    , secure: env.session && env.session.force_ssl
    , path:   '/'
    });

    if (env.session && env.session.force_ssl) {
      env.extras.setCookie('force_ssl', 'true', { maxAge: SESSION_TTL, path: '/' });
    } else {
      env.extras.setCookie('force_ssl', '', { maxAge: 0, path: '/' });
    }
  });
};
