...
module.exports = function (N) {
  ...
  function getSession(sid, callback) {
    N.runtime.redis.get(PREFIX + sid, function (err, data) {
      ...
      N.settings.get('general_session_max_lifetime', {}, function (err, maxLifetime) {
        if (err) {
          callback(err);
          return;
        }

        // Check for max lifetime if it's set. (in days)
        if (0 === maxLifetime || Date.now() < (data.create_time + maxLifetime * 24 * 60 * 60 * 1000)) {
          callback(null, data);
        } else {
          N.runtime.redis.del(PREFIX + sid); // Session expired - delete.
          callback(); // No error, no session.
        }
      });
    });
  ...
  //
  N.wire.after('server_chain:*', { priority: 80, ensure: true }, function session_end(env, callback) {
    if (!env.session_id) {
      callback();
      return;
    }
    ...
    N.settings.get('general_cookies_domain', {}, function (err, domain) {
      ...
      env.extras.setCookie('sid', env.session_id, {
        maxAge: SESSION_TTL
      , domain: domain // note: may be empty === current domain
      , path:   '/'
      });
