...
module.exports = function (N) {
  ...
  N.wire.after('init:models', { priority: 50 }, function emit_init_settings(N, callback) {
    ...
    N.wire.emit('init:settings', sandbox, callback);
  });


  // Normalize setting schemas.
  //
  N.wire.before('init:settings', { priority: -1 }, function normalize_setting_schemas(sandbox, callback) {
    ...
    async.forEach(_.keys(schemasCache), function (name, nextSetting) {
      ...
      async.forEach(schemasCache[name], function (schema, nextSchema) {
        ...
        if (!_.isString(schema.values)) {
          // If `values` is *not* a string - there is nothing more to do.
          nextSchema();
        } else {
          ...
          var fetcher = sandbox.selectionListFetchers[schema.values];
  ...
  //
  N.wire.before('init:settings', { priority: -1 }, function normalize_setting_groups() {
