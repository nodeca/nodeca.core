...
module.exports = function (N) {
  N.wire.before('init:server', function init_mailer(__, wireCallback) {
    ...
    prepare(function (err) {
      if (err) {
        // Mailer initialization error is not fatal and should not stop server
        // startup, so just log an error and continue.
        N.logger.error('Mailer init failure: %s', err);
      } else {
        N.logger.info('Mailer init done');
      }
