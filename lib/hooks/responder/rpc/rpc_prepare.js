...
module.exports = function (N) {
  ...
  function formatParams(params, max) {
    var length = 0
      , result = [];

    function makePair(value, key) {
      return key + '=' + JSON.stringify(value);
    }

    _(params || {}).map(makePair).sortBy('length').forEach(function (pair) {
      if (!max || max > length + pair.length) {
        length += pair.length;
        result.push(pair);
      } else {
        result.push('...');
  ...
  function log(env) {
    ...
    logger[level]('%s - %s%s - "%s" - %s',
                  req.connection.remoteAddress,
                  env.method,
                  formatParams(env.params, 60),
