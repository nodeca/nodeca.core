...
Asset.prototype.__save_cache__ = function (callback) {
  let data = {
    ...
    isBuffer: !!this.__buffer__
  };

  this.__bundler__.cache.put(this.logicalPath, JSON.stringify(data), (err) => {
    if (err) {
      callback(err);
      return;
    }

    this.__bundler__.cache.put(
      this.logicalPath + ':data',
      data.isBuffer ? this.__buffer__ : this.__source__,
      { valueEncoding: data.isBuffer ? 'binary' : 'utf8' },
      callback
    );
  });
};
...
Asset.prototype.__restore_cache__ = function (callback) {
  this.__bundler__.cache.get(this.logicalPath, (err, cache_string) => {
    if (err) {
    ...
    this.__resolveDependencies__((err) => {
      ...
      this.__bundler__.cache.get(this.logicalPath + ':data', (err, src_data) => {
        if (err) {
          if (err.type === 'NotFoundError') {
            callback(null, false);
            return;
        ...
        if (cache.isBuffer) {
          this.__buffer__ = new Buffer(src_data, 'binary');
        } else {
          this.__source__ = src_data;
        }
