...
Asset.prototype.__resolveDependencies__ = thenify.withCallback(function (callback) {
  let assetDeps = _.values(this.__dependencies__).filter(d => d.depType === 'asset');

  let self = this;

  Promise.all(assetDeps.map(p => {
    let asset = self.__bundler__.findAsset(p.logicalPath);

    if (!asset) {
      return Promise.reject(new Error(`Broken dependency ${p.logicalPath} in asset ${self.this.logicalPath}`));
    }
...
Asset.prototype.__save_cache__ = thenify.withCallback(function (callback) {
  let data = {
...
Asset.prototype.__restore_cache__ = thenify.withCallback(function (callback) {
