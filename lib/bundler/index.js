...
//
Bundler.prototype.compile = co.wrap(function* () {
  ...
  yield public_assets.map(asset => asset.resolve());
  ...
  for (let i = 0; i < public_assets.length; i++) {
    ...
    // Write asset
    write.sync(target, buffer);

    // Write gzipped asset
    if (this.compress) {
      let res = yield zlib.gzip(buffer);

      write.sync(target + '.gz', res);
    }

    // Write source map
    if (asset.sourceMap()) {
      // add XSSI protection header
      write.sync(target + '.map', ')]}\'\n' + asset.sourceMap());
    }

    // Write gzipped source map
    if (asset.sourceMap() && this.__options__.compress) {
      let res = yield zlib.gzip(')]}\'\n' + asset.sourceMap());

      write.sync(target + '.map.gz', res);
    }
  }
