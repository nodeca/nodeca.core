...
function flatten_map_sections(sections) {
  let result = [];

  for (let section of sections) {
    if (Array.isArray(section.map.sections)) {
      let { line, column } = section.offset;

      for (let subsection of flatten_map_sections(section.map.sections)) {
        subsection.offset.line += line;
        // eslint-disable-next-line max-depth
        if (line === 0) subsection.offset.column += column;
  ...
  for (let asset of public_assets) {
    ...
    await Promise.all([
      ...
      Promise.resolve().then(async () => {
        ...
        if (map.sections) {
          // browsers have trouble with nested source maps
          map.sections = flatten_map_sections(map.sections);
        }
