...
// Returns whenever or not response content-type is compressable and compression
// is allowed by client
//
// - `'gzip'` if GZip allowed
// - `'deflate'` if Deflate allowed
// - `false` otherwise
//
function get_allowed_compression(req, res) {
  var accept = req.headers['accept-encoding'] || '';

  if (!/json|text|javascript/.test(res.getHeader('Content-Type'))) {
    return false;
  }
...
function finalize(err) {
  ...
  compression = get_allowed_compression(http.req, http.res);
  ...
  compress(compression, resp.body, function (err, buffer) {
    if (err) {
      process_error(http.req, http.res, err);
      return;
    }
