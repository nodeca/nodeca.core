...
function log(code, env, err) {
  var logger  = nodeca.logger.getLogger('server.' + env.request.method),
  ...
  logger[level]('%s - "%s %s HTTP/%s" %d "%s" - %s',
                env.origin.http.req.connection.remoteAddress,
                env.origin.http.req.method,
                env.origin.http.req.url,
                env.origin.http.req.httpVersion,
                code,
                env.origin.http.req.headers['user-agent'],
                message);
}
