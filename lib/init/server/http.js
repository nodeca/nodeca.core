...
function log(level, req, message, data) {
  httpLogger[level](req.method + ' ' + req.url + ' ' + message, data);
}
...
function process_error(req, res, err) {
  ...
  if (err.statusCode) {
    if (400 <= err.statusCode) {
      log('error', req, ' (%d) %s', err.statusCode, err.body);
    }
...
function finalize(err) {
  var http, resp, type, compression;
