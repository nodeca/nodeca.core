...
// adds extensions for `incoming` messages, that allows to subscribe non-server
// clients to the whitelisted channels only
//
...
// adds extensions for `incoming` messages, that allows to publish non-server
// clients to the whitelisted channels only
//
...
function handle_rpc(msg, cb) {
  ...
  nodeca.filters.run(msg.method, msg.params, fn, function (err) {
    var logger  = nodeca.logger.getLogger('server.' + msg.method),
        level   = err ? 'error' : 'info',
        message = (err && err.stack) || (err && err.message) || err || 'OK';

    nodeca.debug_trace('handle_rpc() callback', err);
    logger[level]('"RPC %s(%s) REALTIME" - %s',
                  msg.method, JSON.stringify(msg.params), message);

    (cb || noop)({
      version:  nodeca.runtime.version,
      error:    (err && err.statusCode && 'HTTP ONLY') || String(err) || null,
      result:   (err ? null : JSON.stringify(this.response))
    });
  }, env({
      realtime: true,
      method:   msg.method
