...
// internal
var env         = require('../../env');
var httpLogger  = nodeca.logger.getLogger('http');
...
function handle_rpc(msg, cb) {
  ...
  // log REALTIME requests
  httpLogger.info('REALTIME %s(%j)', msg.method, msg.params);
  ...
  nodeca.filters.run(msg.method, msg.params, fn, function (err) {
    ...
    httpLogger.info('REALTIME %s(%j) -> %s',
                    msg.method, msg.params, err ? 'FAIL' : 'OK');

    (cb || noop)({
      version:  nodeca.runtime.version,
      error:    (err ? err.toString() : null),
      result:   JSON.stringify(this.response)
    });
  }, env({
      realtime: true,
      method:   msg.method
