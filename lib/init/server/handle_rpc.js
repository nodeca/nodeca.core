...
function log(req, res) {
  ...
  if (res.error) {
    ...
    if (!res.error.code || nodeca.io.APP_ERROR <= res.error.code) {
      level = 'fatal';
    } else if (nodeca.io.BAD_REQUEST <= res.error.code && nodeca.io.APP_ERROR > res.error.code) {
      level = 'error';
    } else {
      level = 'info';
    }
...
function process_error(req, res, err) {
  ...
  end(req, res, { code: nodeca.io.APP_ERROR }, null);
}
...
module.exports = function handle_rpc(req, res) {
  ...
  if ('POST' !== req.method) {
    process_error(req, res, nodeca.io.BAD_REQUEST);
    return;
  }
  ...
  req.on('end', function () {
    ...
    // invalid method name
    if (!func) {
      process_error(req, res, nodeca.io.NOT_FOUND);
      return;
    }
