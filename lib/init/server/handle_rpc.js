...
function log(req, res) {
  ...
  if (res.error) {
    ...
    if (!res.error.statusCode || nodeca.io.APP_ERROR <= res.error.statusCode) {
      level = 'fatal';
    } else if (nodeca.io.BAD_REQUEST <= res.error.statusCode && nodeca.io.APP_ERROR > res.error.statusCode) {
      level = 'error';
    } else {
      level = 'info';
    }
...
function process_error(req, res, err) {
  ...
  end(req, res, {statusCode: nodeca.io.APP_ERROR, message: message}, null);
}
...
module.exports = function handle_rpc(req, res) {
  ...
  if ('POST' !== req.method) {
    process_error(req, res, { statusCode: nodeca.io.BAD_REQUEST, body: 'Invalid request method' });
    return;
  }
  ...
  req.on('data', function (chunk) {
    if (MAX_POST_DATA < Buffer.byteLength(data += chunk)) {
      ...
      process_error(req, res, { statusCode: nodeca.io.BAD_REQUEST, body: 'Max POST data reached' });
    }
  });
  ...
  req.on('end', function () {
    ...
    if (!payload.version || !payload.method) {
      process_error(req, res, { statusCode: nodeca.io.BAD_REQUEST, body: 'Invalid payload' });
      return;
    }
    ...
    if (payload.version !== nodeca.runtime.version) {
      process_error(req, res, { statusCode: nodeca.io.BAD_REQUEST, body: 'Client version mismatch' });
      return;
    }
    ...
    // invalid method name
    if (!func) {
      process_error(req, res, { statusCode: nodeca.io.NOT_FOUND, body: 'API path not found' });
      return;
    }
    ...
    // If validation failed, generate error object.
    //
    // - keys -> failed params
    // - values -> fail reason or empty string
    //
    // {
    //    'field1': 'must be int',
    //    'field2': ''
    ...
    if (!validation.valid) {
      ...
      process_error(req, res, { statusCode: nodeca.io.BAD_REQUEST, body: errors });
      return;
    }
