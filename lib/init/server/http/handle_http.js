...
// Replaces localhost IPs with 'localhost' hostname
function get_fixed_hostname(req) {
  // TODO: use default mounting host if no host given
  if (LOCAL_IP_RE.exec(req.headers.host)) {
    return 'localhost' + (RegExp.$1 || '');
  }
...
// Tries to find matching object from router
function find_match(req) {
  var base_url = '//' + get_fixed_hostname(req) + req.pathname, variants, match;

  // FIXME: variants priority list must be 'https, non-prefix` when
  //        `ssl` address is served. otherwise `non-prefix, http`.
  variants = [ base_url, 'https:' + base_url, 'http:' + base_url ];

  while (!match && variants.length) {
    match = nodeca.runtime.router.match(variants.shift());
  }
