...
function process_error(req, res, err, env) {
  ...
  if (err.statusCode) {
    err.headers = _.defaults(err.headers || {}, {'Content-Type': 'application/json; charset=utf-8'});
  ...
  end(req, res, 500, {'Content-Type': 'application/json; charset=utf-8'}, body, env, err);
}
...
function finalize(err) {
  ...
  if (false === compressor || 500 > Buffer.byteLength(resp)) {
    end(http.req, http.res, resp.statusCode || 200, resp.headers, resp, env);
    return;
  }
  ...
  compression.process(compressor, resp, function (err, buffer) {
    if (err) {
...
module.exports = function handle_rpc(req, res) {
  ...
  // invalid payload
  if (!payload.id || !payload.version || !payload.method) {
    process_error(req, res, {statusCode: 400});
    return;
  }
  ...
  if (payload.version !== nodeca.runtime.version) {
    // handle error
    // {version: nodeca.runtime.version, error: 'Client version mismatch'}
  ...
  nodeca.filters.run(payload.method, payload.params || {}, func, finalize, env({
    rpc:    {req: req, res: res},
