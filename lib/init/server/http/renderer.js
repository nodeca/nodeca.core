...
// Returns whenever or not response (for the `req`uest can be compressed)
//
...
module.exports = function renderer(params, callback) {
  ...
  views = nodeca.runtime.views[locale][theme];

  if (!nodeca.shared.common.getByPath(views, this.response.view)) {
  ...
  try {
    ...
    this.response.body = nodeca.shared.common.render(views, this.response.view, layout, locals);
  } catch (err) {
    callback(err);
    return;
  }
