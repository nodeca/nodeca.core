...
Extras.prototype = {
  requireJavascript: function (logicalPath) {
    ...
    if (nodeca.runtime.assets.environment.findAsset(logicalPath)) {
      this.__data__.precompile.push(logicalPath);
    }
  },
...
nodeca.filters.after('', {weight: 50}, function precompile_required_assets(params, callback) {
  if (0 === this.extras.assets.__data__.precompile.length) {
    callback(null);
    return;
  }

  nodeca.runtime.assets.environment.precompile(this.extras.assets.__data__.precompile, callback);
});
