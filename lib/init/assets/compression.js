...
if (process.env.NODECA_ASSETS_DEBUG) {
  debug = function debug(result, file, mime) {
    console.log('[COMPRESSOR CACHE] *** (%s) *** %s / %s', result, file, mime);
  };
}
...
function cachable(compressor) {
  return function (context, data, callback) {
    ...
    if (cached.digest === digest) {
      debug('HIT', context.logicalPath, context.contentType);
      callback(null, cached.data);
      return;
    }

    try {
      debug('MISS', context.logicalPath, context.contentType);
