...
function start_server(next) {
  ...
  // TODO: start server on all requested points

  // create server
  host    = nodeca.config.router.bind['default'].listen.split(':').shift() || 'localhost';
  port    = nodeca.config.router.bind['default'].listen.split(':').pop() || 3000;
