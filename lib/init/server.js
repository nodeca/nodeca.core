...
module.exports = function (next) {
  ...
  // dummy helper that skips hosts validation test
  //
  function skipFilterHosts(req, res, callback) {
    callback();
  }
  ...
  async.forEachSeries(_.keys(bindings), function (point, nextPoint) {
    ...
    (options.ssl ? startSecureServer : startServer)(options, function (err, server) {
      var filterHosts = skipFilterHosts;
      ...
      // if address:port has no 'any' host mount points,
      // check request host before any actual processings
      if (!_.include(options.hosts, '*')) {
        filterHosts = function filterHosts(req, res, callback) {
      ...
      listener.attach(server, filterHosts, nextPoint);
    });
  }, next);
};
