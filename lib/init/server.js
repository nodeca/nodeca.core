...
module.exports = function (next) {
  ...
  async.forEachSeries(_.keys(bindings), function (point, nextPoint) {
    var options = bindings[point],
        ...
        port    = options.port,
        server  = options.ssl ? https.createServer(options) : http.createServer(),
        host_is_valid;
    ...
    function handle_server_error(err) {
      ...
      if ('EADDRINUSE' === err.code) {
        nextPoint(err_prefix + 'Address in use...');
        return;
      }

      if ('EADDRNOTAVAIL' === err.code) {
        // system has no such ip address
        nextPoint(err_prefix + 'Address is not available...');
        return;
      }

      if ('ENOENT' === err.code) {
        // failed resolve addressname to ip address
        nextPoint(err_prefix + "Failed to resolve IP address...");
        return;
      }

      // unexpected / unknown error
      nextPoint(err_prefix + err);
    }
