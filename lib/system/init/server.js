...
module.exports = function (N) {
  ...
  N.wire.on('init:server', function* cluster_init(N) {
    ...
    if (cluster.isMaster && (fork >= 1)) {
      ...
      N.wire.on('shutdown', { ensure: true }, function* shutdown_workers() {
        shutting_down = true;

        yield pools.map(pool => new Promise(resolve => {
          function on_exit() {
            // check workers in every state except DEAD
            if (pool.get([ 'INIT', 'START', 'RUN', 'PEND', 'STOP', 'KILL' ]).length === 0) {
      ...
      N.wire.on('terminate', { ensure: true }, function* terminate_workers() {
        shutting_down = true;
