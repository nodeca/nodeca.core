...
WorkerPool.prototype = {
  ...
  terminate() {
    ...
    this.__workers.forEach(w => { w.terminate(); });
  },
...
Worker.prototype = {
  ...
  terminate() {
    if (this.__w) this.__w.kill('SIGKILL');
    else this.die();
  },
  ...
  onSTART() {
    ...
    setTimeout(() => {
      if (!rcvd_startup_message) this.terminate();
    }, 30000);
  },

  onSTOP() {
    this.__w.kill('SIGTERM');
    setTimeout(() => this.terminate(), 15000);
  },
