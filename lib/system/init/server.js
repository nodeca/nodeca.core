...
module.exports = function (N) {
  ...
  N.wire.on('init:server.worker', function* server_bind(N) {
    let bindings = extractBindings(N.config.bind, N.mainApp, N.logger);
    let points = _.keys(bindings);

    function bindServer(point) {
      ...
      return new Promise((resolve, reject) => {
        ...
        setTimeout(function () {
          reject(`Failed to bind '${point}' (timed out)`);
        }, 3000);
