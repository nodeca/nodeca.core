...
function extractBindings(config, mainApp, logger) {
  ...
  _.forEach(config, function (options) {
    ...
    if (!result[options.ssl.listen]) {
      ...
      // Read files.
      [ 'key', 'cert' ].forEach(function (val) {
        if (!options.ssl[val]) { return; }

        var filename = path.resolve(mainApp.root, options.ssl[val]);

        try {
          result[options.ssl.listen].ssl[val] = fs.readFileSync(filename, 'utf8');
        } catch (err) {
          logger.error('Can\'t read %s-file from SSL config (%s): %s', val, filename, err);
        }
      });
    }
  });
  ...
  _.forEach(config, function (options) {
    var host = parseURL(options.mount, false, true).host || '*';
...
module.exports = function (N) {
  ...
  N.wire.on('init:server', function models_init(N, cb) {
    var bindings = extractBindings(N.config.bind, N.mainApp, N.logger);

    //
    // bind listeners
    //

    async.eachSeries(_.keys(bindings), function (point, next) {
      ...
      if (_.contains(options.hosts, '*')) {
        ...
        host_is_valid = function (host) {
          return _.contains(options.hosts, host);
        };
      }
      ...
      server.on('request', function handle_request(req, res) {
        var protocol, isEncrypted, httpMethod, parsedUrl, fullUrl, type, routeMatch, remoteAddress;
        ...
        if (parsedUrl.pathname === '/io/rpc') {
          ...
          routeMatch = _.find(N.router.matchAll(fullUrl), function (match) {
      ...
      N.wire.emit(options.ssl ? 'init:server.https' : 'init:server.http', server, function (err) {
        if (err) {
          next(err);
          return;
        }
