  ...
  N.wire.on("init:server", function models_init(N, cb) {
    ...
    async.forEachSeries(_.keys(bindings), function (point, next) {
      ...
      server.on('request', function handle_request(req, res) {
        var isEncrypted, httpMethod, parsedUrl, type, routeMatch;
        ...
        // but connect with us via plain HTTP.
        isEncrypted = (N.config.options.server.respect_forwarded_proto &&
        ...
        req.fullUrl  = (isEncrypted ? 'https': 'http') + '://' +
