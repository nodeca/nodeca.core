...
// Parses URL query string and resolves numbers, booleans and nulls.
//
function parseQueryString(inputQuery) {
  ...
  function resolveValue(inputValue) {
    var parsedValue;

    if (_.isArray(inputValue)) {
      return _.map(inputValue, resolveValue);
    ...
    } else if (/^-?[0-9]+(:?\.[0-9]*)?$/.test(inputValue)) {
  ...
  parsedQuery = querystring.parse(String(inputQuery));

  _.forEach(parsedQuery, function (value, key) {
