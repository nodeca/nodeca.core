    ...
    if (!result[options.ssl.listen]) {
      ...
      _(options.ssl).pick('key', 'cert').map(function (filename, key) {
        ...
        try {
          ...
          logger.error('Can\'t read %s-file from SSL config (%s): %s', key, filename, err);
        }
      });
    }
  });
...
module.exports = function (N) {
  ...
  N.wire.on('init:server', function models_init(N, cb) {
    var bindings = extractBindings(N.config.bind, N.runtime.mainApp, N.logger),
    ...
    async.eachSeries(_.keys(bindings), function (point, next) {
      ...
      server.on('request', function handle_request(req, res) {
        ...
        // hard limit max request size to 20 mb
        // user should not be able to upload temporary files of unlimited size
        var received = 0;
        var maxRequestSize = 20 * 1024 * 1024;
