...
module.exports = function (sandbox, callback) {
  ...
  _.forEach(sandbox.config.packages, function (pkg) {
    if (pkg.client) {
      clientModules = clientModules.concat(_.invoke(pkg.client.files, 'toString'));
    }
  });
  ...
  async.forEachSeries(_.keys(sandbox.config.packages), function (pkgName, next) {
    ...
    try {
      ...
      if (_.find(clientConfig, 'main')) {
        browserifyMainFile(N, _.find(clientConfig, 'main').main, mainFile);
        targetFile = mainFile;
      } else {
