...
// Check if there is any existent and bundled vendor module by client module
// identifier (absolute path or an alias name).
function hasVendorModule(sandbox, modulePath) {
  var moduleExt, found = false;

  moduleExt  = path.extname(modulePath);
  modulePath = path.basename(modulePath, moduleExt);
  ...
  _.each(sandbox.packages, function (pkgConfig) {
    ...
    _.each(pkgConfig.vendor, function (vendorPath) {
      var vendorExt;
      ...
      vendorExt  = path.extname(vendorPath);
      vendorPath = path.basename(vendorPath, vendorExt);

      // If the module we looking for has no explicit extension, accept any
      // available extension on vendor modules.
      if (('' === moduleExt || moduleExt === vendorExt) &&
...
function browserify(sandbox, files, destination) {
  ...
  _.each(files, function (pathname) {
    ...
    findRequires(source).forEach(function (modulePath) {
      ...
      if (!hasVendorModule(modulePath)) {
        ...
        browserify(sandbox, [moduleFile], destination);
      }
    });
