  ...
  async.forEachSeries(_.keys(sandbox.config.packages), function (pkgName, next) {
    ...
    try {
      ...
      if (clientConfig.main) {
        mainFile = clientConfig.main.toString();
        browserifyMainFile = true;
      } else {
        mainFile = clientTmpTreefile;
        browserifyMainFile = false;
      }
    } catch (err) {
      next(err);
      return;
    ...
    environment.findAsset(mainFile).compile(function (err, data) {
      ...
      if (browserifyMainFile) {
        source = browserifySource(source, {
          fsPath: mainFile
