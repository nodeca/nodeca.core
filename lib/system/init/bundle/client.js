...
// Contains full map of vendor aliases of current sandbox.
var vendorAliases;
...
function browserifySingle(N, source, options) {
  ...
  findRequires(source, { raw: true }).forEach(function (match) {
    ...
    if (vendorAliases[firstPathNode]) {
      // Resolve alias path.
      resolvedPath = path.resolve(vendorAliases[firstPathNode],
...
module.exports = function (sandbox, callback) {
  ...
  _.forEach(sandbox.config.packages, function (pkg) {
    _.forEach(pkg.vendor, function (file, alias) {
      if (alias) {
        vendorAliases[alias] = file;
      }
    });
  });
