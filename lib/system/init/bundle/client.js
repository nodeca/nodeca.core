...
function collectBundledModules(sandbox) {
  ...
  _.forEach(sandbox.config.packages, function (pkgConfig) {
    if (pkgConfig.vendor) {
      vendorModules = _.union(vendorModules, _.values(pkgConfig.vendor));
    }

    if (pkgConfig.client) {
      clientModules = _.union(clientModules, _.invoke(pkgConfig.client.files, 'toString'));
    }
  });
}
...
function browserifySingle(source, options) {
  ...
  findRequires(source, { raw: true }).forEach(function (match) {
    ...
    if (!fs.existsSync(resolvedPath)) {
      if (skipMissing) {
        ...
        throw new Error(
          ...
          ' contains require of a non-existent module: '   +
          '"' + resolvedPath + '"'
        );
      }
    }
    ...
    if (_.contains(clientModules, resolvedPath)) {
      throw new Error(
        (fsPath || 'Some file (the location is unknown)') +
        ' contains require of a client module ' +
        '"' + resolvedPath + '"'
      );
    }
