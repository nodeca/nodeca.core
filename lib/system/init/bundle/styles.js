...
function joinStyles(lookup, destination, options) {
  ...
  var fileDigest = _.memoize(function (file) {
    return strDigest(fs.readFileSync(file, 'utf-8'));
  });
  ...
  findPaths(lookup, function (file) {
    ...
    if (null !== cache.resultCSS) {
      // Check dependencies
      if(_.any(cache.dependencies, function(digest, file){
          // Compare digests
    ...
    if (null === cache.resultCSS) {
      // Since cached result isn't a valid,
      // we need invoke suitable renderer and update cache

      // Invoke renderer
      var rendered = renderer(file, options);

      cache.resultCSS = rendered.css;

      // Update dependencies
      cache.dependencies = _.transform(rendered.imports, function(dependencies, file){
