...
//
function concatStyles(lookup, destination, options, callback) {
  var pathnames = findPaths(lookup);
  ...
  fstools.mkdir(path.dirname(destination), function (err) {
    ...
    async.mapSeries(pathnames, function (pathname, next) {
      var render = RENDERERS[path.extname(pathname.fsPath)];

      if (!render) {
        next("Don't know how to compile " + pathname.fsPath);
        return;
      }

      render(pathname.fsPath, options, next);
    }, function (err, chunks) {
      if (err) {
        callback(err);
        return;
...
module.exports = function (sandbox, callback) {
  ...
  async.forEachSeries(_.keys(sandbox.config.packages), function (pkgName, next) {
    ...
    concatStyles(stylesConfig, stylesTmpTreefile, concatOptions, function (err) {
      var
      // if config has "main file", the respect it, otherwise use styles tree only
      mainFile    = _.find(stylesConfig, 'main') ? _.find(stylesConfig, 'main').main : stylesTmpTreefile,
