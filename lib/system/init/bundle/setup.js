...
module.exports = function (sandbox, callback) {
  ...
  //
  // TODO: extend with global i18n changes
  ...
  bundler.registerHelper('asset_url', function asset_path(path) {
    let resolved_path = require.resolve(path);
    let asset = bundler.findAsset(resolved_path);

    if (!asset) {
      throw new Error(`Invalid asset path ${resolved_path}`);
    }
  ...
  search_assets(N.apps, function (err, config) {
    if (err) {
      callback(err);
      return;
    }
