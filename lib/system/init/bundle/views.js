...
module.exports = function (sandbox) {
  var clientDir = path.join(sandbox.tmpdir, 'views')
    , timer     = stopwatch();
  ...
  _.forEach(sandbox.config.packages, function (pkgConfig, pkgName) {
    ...
    findPaths(pkgConfig.views, function (fsPath, apiPath) {
      ...
      sandbox.N.views[apiPath] = render.server(string, { filename: fsPath });
  ...
  sandbox.N.logger.info('Processed views section %s', timer.elapsed);
};
