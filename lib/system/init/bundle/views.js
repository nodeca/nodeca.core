...
var TEMPLATE_PATH = path.join(__dirname, 'views', 'wrapper.tpl');
...
//  writeClientViews(filename, views, callback(err)) -> String
...
//      });
//
function writeClientViews(filename, views, callback) {
  fstools.mkdir(path.dirname(filename), function (err) {
    ...
    fs.writeFile(filename, TEMPLATE({ views: views }), 'utf8', callback);
  });
}
...
function processViews(N, outfile, config, callback) {
  var serverViews = {}
    , clientViews = {}
    , pathnames   = findPaths(config);

  async.forEachSeries(pathnames, function (pathname, next) {
    var str;

    try {
      str = fs.readFileSync(pathname.fsPath);
    } catch (err) {
      next(err);
      return;
    }
    ...
    async.parallel([
      function (next) {
        ...
        engines[path.extname(pathname.fsPath)].server(str, {
          filename: pathname.fsPath
        }, next);
      },
      function (next) {
        // compile client-side views (string sources of functions)
        engines[path.extname(pathname.fsPath)].client(str, {
          filename: pathname.fsPath
        }, next);
      }
    ...
    writeClientViews(outfile, clientViews, callback);
  });
}
