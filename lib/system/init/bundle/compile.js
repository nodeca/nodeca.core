...
// Compose full list of packages are needed to load in order to load the given
// package in the right order. That list will include the given package at the
// last position.
//
function composePackageQueue(sandbox, mainPkgName) {
  var result = [];
...
function createLoaderPkgsMap(sandbox, bundleAssets) {
  ...
  N.config.locales["enabled"].forEach(function (locale) {
    ...
    _.each(sandbox.config.packages, function (pkgConfig, pkgName) {
      ...
      distribution[locale][pkgName] = {
        loadingQueue: composePackageQueue(sandbox, pkgName),
        stylesQueue:  [],
        stylesheet:   null,
        javascript:   null
      };
  ...
  //
  // After the distribution is compiled, we need to compose stylesheet queues
  // for each existing package with respect to the package dependences. That is
  // used in the view layouts for linking initial page stylesheets.
  //

  _.each(distribution, function (localeDist) {
    _.each(localeDist, function (pkgDist) {
      pkgDist.loadingQueue.forEach(function (depName) {
        var depDist = localeDist[depName];

        if (depDist && depDist.stylesheet) {
          if (-1 === pkgDist.stylesQueue.indexOf(depDist.stylesheet)) {
            pkgDist.stylesQueue.push(depDist.stylesheet);
          }
        }
      });
    });
