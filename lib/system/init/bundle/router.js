...
module.exports = function (sandbox, callback) {
  ...
  unknown_methods = _.filter(_.keys(router_config.map || {}), function (api_path) {
    return !(N.wire.has('server:' + api_path) ||
             N.wire.has('server:' + api_path + ':*') ||
             N.wire.has('server:' + api_path + ':http') ||
             N.wire.has('server:' + api_path + ':rpc'));
  });
  ...
  N.logger.info('Processed routes ' + timer.elapsed);
