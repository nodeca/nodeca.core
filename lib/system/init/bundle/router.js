...
function prepareMountingConfig(config) {
  ...
  //
  var mountDefaults = url.parse(config.default.mount, false, true);
  ...
  _.forEach(config, function (options, key) {
    ...
    var mount = url.parse(options.mount, false, true);
  ...
  _.forEach(config, function (options) {
    ...
    if (options.listen && options.ssl) {
      _.forEach(config, function (other) {
...
module.exports = function (N) {
  ...
  _.forEach(N.config.router, function (serverMethods, optionsString) {
    ...
    if (_.includes(typesList, 'get') && !_.includes(typesList, 'head')) {
  ...
  // Expose the result.
  N.router = new Pointer();

  _.forEach(routeConfigs, function (route) {
