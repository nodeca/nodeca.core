// Search files using bundle conditions (root, include, exclude, main).
//
...
function listFilter(arr) {
  ...
  return function (file) {
    return _(arr).map(function (test) {
...
//
function toArray() {
  return _(arguments).flatten().filter(Boolean).valueOf();
}
...
function findPaths(options, callback) {
  ...
  fstools.walk(root, function (file, stats, next) {
    var relative = file.substr(root.length).replace(/^\/+/, '')
      ...
      , relativeApiPath  // e.g. "blocks.single_post"
      , completeApiPath; // pkgName + method, e.g. "forums.blocks.single_post"

    if (null !== matched) {
      relativeApiPath = apify(matched);

      if (relativeApiPath) {
        completeApiPath = pkgName + '.' + relativeApiPath;
      } else {
        completeApiPath = pkgName;
      }

      pathnames.push(new Pathname(file, {
        apiPath:  deduplicateApiPath(completeApiPath),
