...
// Returns normalized version of vendor section of a package. Resolves all of
// the paths to vendor files relative to `rootDir` using `resolveModulePath`.
function normalizeVendorSection(vendorSection, rootDir) {
  var result = {}, resolvedPath;

  _.each(vendorSection || [], function (vendorPkg) {
    ...
    if (_.isPlainObject(vendorPkg)) {
      ...
      if (1 !== asPair.length) {
        throw 'Ill-formed list of vendor files.';
      }
    ...
    resolvedPath = resolveModulePath(rootDir, depPath);
        ...
        _.each(app_config.packages, function (pkgConfig, pkgName) {
          ...
          config.packages[pkgName].vendor =
            _.extend((config.packages[pkgName].vendor || {}),
                     normalizeVendorSection(pkgConfig.vendor, app.root));
          ...
          _(pkgConfig).pick(FILE_COLLECTOR_CONFIG_SECTIONS).each(function (sectionConfig, sectionName) {
            ...
            if (sectionConfig.main) {
              ...
              if (!_.isArray(c.exclude)) {
                c.exclude = [ c.exclude, c.main.toString() ];
              } else {
                c.exclude.push(c.main.toString());
              }
            }
  ...
  async.forEachSeries(_.keys(config.packages), function (pkgName, next) {
    ...
    async.forEachSeries(_.keys(config.packages[pkgName]), function (key, next) {
      if (_.contains(FILE_COLLECTOR_CONFIG_SECTIONS, key)) {
        try {
          config.packages[pkgName][key].files = findPaths(config.packages[pkgName][key].lookup);
          next();
        } catch (err) {
          next(err);
        }
