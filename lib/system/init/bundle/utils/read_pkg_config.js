...
// internal
var resolveVendorPackages = require('./resolve_vendor_packages');
        ...
        _.each(app_config.packages, function (pkgConfig, pkgName) {
          ...
          config.packages[pkgName].vendor =
            _.extend((config.packages[pkgName].vendor || {}),
                     resolveVendorPackages(app.root, pkgConfig.vendor));

          // merge common resource declarations
          _.chain(pkgConfig).omit('depends', 'vendor').each(function (sectionConfig, sectionName) {
            var c, lookup;
