...
function normalizePackages(app, config) {
  ...
  _.forEach(config, function (pkgConfig, pkgName) {
    result[pkgName] = {
      depends:
        pkgConfig.depends ?
          Array.isArray(pkgConfig.depends) ? pkgConfig.depends : [ pkgConfig.depends ]
        :
          [],
    ...
    RESOURCE_NAMES.forEach(function (resource) {
...
function mergeConfig(target, source) {
  if (Array.isArray(target)) {
    (Array.isArray(source) ? source : [ source ]).forEach(function (object) {
      if (object && target.indexOf(object) < 0) {
        target.push(object);
      }
    });
  } else {
...
module.exports = function (applications) {
  ...
  applications.forEach(function (app) {
    ...
    config = yaml.safeLoad(data, { filename: file });
  ...
  var pkgNames = appConfigs
                    .reduce(function (acc, app) {
                      Object.keys(app.packages || {}).forEach(function (pkg) {
                        if (acc.indexOf(pkg) < 0) { acc.push(pkg); }
                      });
                      return acc;
                    }, []);
  ...
  pkgNames.forEach(function (pkgName) {
    RESOURCE_NAMES.forEach(function (resource) {
      ...
      appConfigs.forEach(function (config) {
        ...
        if (pkgConfig && pkgConfig[resource] && pkgConfig[resource].main) {
          if (hasMain) {
  ...
  mergedConfigs = appConfigs.reduce(mergeConfig, {});

  // Ensure existence of the declared dependences.
  Object.keys(mergedConfigs.packages).forEach(function (pkgName) {
    mergedConfigs.packages[pkgName].depends.forEach(function (depName) {
