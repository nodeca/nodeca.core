...
var statCached   = require('./stat_cached');
 ...
 * - compact (Boolean) - set `false` to keep records without hits on save
 * - autosave - delay in ms to autosave cache state. 500 by default, set false or 0 to disable.
 **/
...
CachedProcessor.prototype.save = function () {
  ...
  fstools.mkdirSync(path.dirname(this.cachePath));
  fs.writeFileSync(this.cachePath, JSON.stringify(to_save));
};
