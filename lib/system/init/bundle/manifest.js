...
module.exports = function (sandbox) {
  ...
  var missed =  _.find(sandbox.assets.used, function (assetPath) {
    return sandbox.assets.files.indexOf(assetPath) === -1 ? true : false;
  });
  ...
  fileslist = sandbox.assets.files
                .map(function (f) {
                  return (environment.findAsset(f) || {}).logicalPath;
                })
                // filter unique non empty
                .reduce(function (acc, val) {
                  if (val && acc.indexOf(val) < 0) {
                    acc.push(val);
                  }
                  return acc;
                }, []);
  ...
  server.log = function logAssets(level, event) {
    logger[level]('%s - "%s %s HTTP/%s" %d "%s" - %s',
                  event.request.ip || event.remoteAddress,
                  event.method,
