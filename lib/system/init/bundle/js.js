...
module.exports = function (sandbox) {
  ...
  for (let pkg of Object.values(sandbox.config.packages)) {
    for (let name of Object.keys(pkg.vendor)) {
      exclude.push(name);
    }
  }
  ...
  async function browserify_concat_plugin(context) {
    if (!context.asset.source.trim()) return;

    let b = browserify({
      prelude: 'NodecaLoader.wrap',
      detectGlobals: true, // @popper/core uses process.env
      debug: context.bundler.sourceMaps // adds inline source maps
    });

    for (let file_path of context.asset.__queue__) {
    ...
    b.on('dep', function (row) {
      if (row.entry) {
        for (let [ dep_name, index ] of Object.entries(row.indexDeps)) {
          if (index) {
