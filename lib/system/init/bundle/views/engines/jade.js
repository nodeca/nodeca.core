...
exports.server = function (file) {
  var content = fs.readFileSync(file, 'utf8');

  var tpl = jade.compileClient(content, {
  ...
  // create closure with runtime
  return  'var jade = require("jade/lib/runtime");\n' +
          tpl + '\n' +
          'return template;\n';
};


exports.client = function (file) {
  var content = fs.readFileSync(file, 'utf8');

  return jade.compileClient(content, {
    // debug decrease speed and disable external cache
    // (injects unique paths every time, because of random tml dir)
    compileDebug: false,
    // NO `with` -> better speed
