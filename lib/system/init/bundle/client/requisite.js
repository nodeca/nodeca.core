...
Requisite.prototype.process = function (source, pathname) {
  ...
  findRequires(source, { raw: true }).forEach(function (info) {
    ...
    try {
      // resolve as node module into absolute path
    ...
    try {
      // resolve path relatively to pathname (it will not affect absolute paths)
      // if it was resolved as node module path
      requiredPath = require.resolve(path.resolve(base, requiredPath));
    } catch (err) {
      throw new Error("Can't resolve required path " + JSON.stringify(info.raw) +
                      " in " + pathname + ":" + info.line);
    }
