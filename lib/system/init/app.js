  ...
  readPkgConfig(N.runtime.apps, function (err, config) {
    ...
    N.hooks.init.run('application', function (next) {
      async.series([
        ...
        step('bundle-compile',  require('./app/manifest'),  tmpdir, sandbox),
        step('stores-section',  require('./app/stores'),    tmpdir, sandbox),
        step('models-section',  require('./app/models'),    tmpdir, sandbox),
