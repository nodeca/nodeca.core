...
function prepareProcessingQueue(sandbox) {
  return PROCESSING_QUEUE.map(function (modulePath) {
  ...
  N.wire.on('init:bundle', function bundle_all(N, callback) {
    ...
    process.on('exit', function (code) {
      ...
      try {
        ...
        N.logger.error('Failed to remove %s, %s ', tmpdir, String(e));
      }
    });

    try {
      sandbox.tmpdir = tmpdir = fstools.tmpdir();
      sandbox.config = readPkgConfig(N.apps);
      fs.mkdirSync(tmpdir);
    } catch (e) {
      err = e;
    }
    ...
    // Sync calls simplify profiling. Old code is commented for
    // rollback possibility, if one of processor become async.
    /*try {
      PROCESSING_QUEUE.forEach(function(processor) {
        require(processor)(sandbox);
      });
    } catch (e) {
      err = e;
    }
