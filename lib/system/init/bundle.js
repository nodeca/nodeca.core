...
module.exports = function (N) {
  ...
  function init_server(N, api_path_prefix, root_path) {
    glob('**/*.js', { cwd: root_path, nodir: true }).filter(name => !/^[._]|\\[._]|\/[_.]/.test(name)).forEach(name => {
      ...
      try {
        ...
        // Skip errors, but log it
        N.logger.error(`Server modlue "${file_path}" has error: ${e}`);
        return;
      }
      ...
      let api_path_start = path.dirname(name).replace(new RegExp(path.sep, 'g'), '.');
  ...
  N.wire.on('init:bundle', function bundle_all(N, callback) {
    ...
    async.eachSeries(PROCESSING_QUEUE, (task, cb) => {
      let timer = stopwatch();
