// Main application initializer. Executed as the most first step of the
// `cli/server.js` and does following:
//
//  - fetches all bin files of all packages
//  - compiles styles client trees for all packages
//  - find and read all viewws
//  - find and prepare all server methods
//  - prepares bundles client js files for all packages containing:
//    - i18n of the package
//    - client methods
//    - server methods wrapers
//    - views
//  - output bundled files into the `public/assets` dir
//  - initialize router


'use strict';


// stdlib
var fs = require('fs');


// 3rd-party
var async   = require('async');
var fstools = require('fs-tools');


// internal
var stopwatch     = require('./utils/stopwatch');
var readPkgConfig = require('./bundle/utils/read_pkg_config');


////////////////////////////////////////////////////////////////////////////////


module.exports = function (N) {
  var timer = stopwatch();

  N.wire.on("init:bundle", function bundle_all(N, callback) {
    var tmpdir;

    N.logger.info('Bundler: started');

    // schedule files cleanup upon normal exit
    process.on('exit', function (code) {
      if (0 !== +code) {
        console.warn("Unclean exit. Bundled files left in '" + tmpdir + "'");
        return;
      }

      try {
        N.logger.debug("Removing '" + tmpdir + "'...");
        fstools.removeSync(tmpdir);
      } catch (err) {
        N.logger.error("Failed remove '" + tmpdir + "'... " + String(err));
      }
    });

    try {
      // create temporary dir for styles
      tmpdir = fstools.tmpdir();
      fs.mkdirSync(tmpdir);
    } catch (err) {
      callback(err);
      return;
    }

    readPkgConfig(N.runtime.apps, function (err, config) {
      var sandbox = {
        N:      N,
        tmpdir: tmpdir
      };

      if (err) {
        callback(err);
        return;
      }

      sandbox.config = config;

      async.series([
        async.apply(require('./bundle/mincer'),    sandbox),
        async.apply(require('./bundle/bin'),       sandbox),
        async.apply(require('./bundle/styles'),    sandbox),
        async.apply(require('./bundle/i18n'),      sandbox),
        async.apply(require('./bundle/views'),     sandbox),
        async.apply(require('./bundle/server'),    sandbox),
        async.apply(require('./bundle/router'),    sandbox),
        // client depends on router -- as we are bundling routes into N
        async.apply(require('./bundle/client'),    sandbox),
        async.apply(require('./bundle/compile'),   sandbox),
        async.apply(require('./bundle/manifest'),  sandbox)
      ], function (err) {
        if (err) {
          callback(err);
          return;
        }

        N.logger.info('Bundler: finished ' + timer.elapsed);
        callback();
      });
    });
  });
};
