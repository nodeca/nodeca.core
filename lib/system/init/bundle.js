...
module.exports = function (N) {
  ...
  N.wire.on('init:bundle', async function bundle_all(N) {
    ...
    let bundler = new Bundler({
      ...
      brotli: N.environment !== 'development' && process.env.NODECA_NOBROTLI !== '1'
    });
  ...
  N.wire.after('init:bundle', function load_server_assets(N) {
    ...
    for (let [ logical_path, asset_info ] of Object.entries(manifest.files)) {
      if (!logical_path.startsWith('server/')) continue;

      // TODO: remove this check when components will be virtual
      // assume that digestPath has forward slashes even on windows
      if (asset_info.digestPath.startsWith('server/package-component-')) {
        continue;
      }
  ...
  N.wire.after('init:bundle', function assets_helpers_add(N) {
    ...
    for (let [ logical_path, asset_info ] of Object.entries(N.assets.files)) {
      // assume that digestPath has forward slashes even on windows
      if (!asset_info.digestPath.startsWith('public/')) continue;
