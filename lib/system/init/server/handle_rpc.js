...
module.exports = function handle_rpc(_N, req, res) {
  var data = '';

  // Lazy init
  if (!N) {
    N = _N;
    logger = N.logger.getLogger('rpc');
  }
  ...
  req.on('end', function () {
    ...
    // invalid method name
    if (!N.wire.has('server:' + payload.method)) {
      process_error(req, res, N.io.NOT_FOUND);
      return;
    }

    validation = N.validate.test('server:' + payload.method, params);
    ...
    N.wire.emit('server:' + payload.method, _env, function (err) {
      if (err) {
        process_error(req, res, err);
        return;
      }

      end(req, res, null, _env.response);
    });
  });
};
