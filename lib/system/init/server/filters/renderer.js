...
//
N.server.after('**', { priority: 85 }, function renderer(env, callback) {
  ...
  try {
    locals = _.extend(env.response.data, helpers, env.helpers, {runtime: env.runtime});
    env.response.body = env.helpers.render(env.response.view, locals, layout);
  } catch (err) {
