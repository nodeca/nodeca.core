...
// Post-process response data: tweak headers & compression
//
function process_response(env, err) {
  var http, resp, type, compressor;

  if (err) {
    process_error(env.origin.http.req, env.origin.http.res, err, env);
    return;
  }
...
module.exports = function handle_http(_N, req, res) {
  // Lazy init
  if (!N) {
    N = _N;
  }
  ...
  validation = N.validate.test('server:' + match.meta, params);
  ...
  N.wire.emit('server:' + match.meta, _env, function (err) {
    process_response(_env, err);
  });
};
