...
function distributeAssets(sandbox, bundleAssets) {
  ...
  //
  // calculates and returns assets for specific pkgName
  //

  function fetch(pkgName) {
    if (!distribution[pkgName]) {
      distribution[pkgName] = {};

      N.config.locales["enabled"].forEach(function (locale) {
        distribution[pkgName][locale] = {
          stylesheets: [],
          javascripts: []
        };
        ...
        pkgBundles[pkgName].forEach(function (bndlName) {
          ...
          if (stylesheet) {
            ...
            distribution[pkgName][locale].stylesheets.push(stylesheet);
          ...
          if (javascript) {
            ...
            distribution[pkgName][locale].javascripts.push(javascript);
  ...
  //
  // collect assets for each package
  //

  _.each(sandbox.config.packages, function (_, pkgName) {
    distribution[pkgName] = fetch(pkgName);
  });
