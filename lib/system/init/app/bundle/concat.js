...
//
function concatPackageJavascripts(pkgName, tmpdir, callback) {
  // collect package "base" javascripts
  async.map([ 'server', 'views', 'client' ], function (part, next) {
...
function collectPackageAssets(tmpdir, sandbox, callback) {
  ...
  async.forEach(_.keys(sandbox.config.packages), function (pkgName, next) {
    async.series([
      ...
      async.apply(concatPackageJavascripts, pkgName, tmpdir)
    ], function (err, results) {
      if (err) {
        next(err);
        return;
