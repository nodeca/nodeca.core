...
function isCurrentMonth(date) {
  var now = new Date();
  return new Date(date.setDate(1)).toDateString() === new Date(now.setDate(1)).toDateString();
}
...
DateFormatter.prototype.__format_date__ = function __format_date__(date, pattern) {
  ...
  return pattern.replace(REPLACE_RE, function (match, fmt) {
    ...
    switch (fmt) {
      ...
      case '=a':
        return self.cldr.days['stand-alone'].abbreviated[day_keys[date.getDay()]];
      case '=A':
        return self.cldr.days['stand-alone'].wide[day_keys[date.getDay()]];
      case '=b':
      case '=h':
        return self.cldr.months['stand-alone'].abbreviated[date.getMonth() + 1];
      case '=B':
        return self.cldr.months['stand-alone'].wide[date.getMonth() + 1];
