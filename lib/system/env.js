...
module.exports = function env(N, options) {
  ...
  //
  var findBindConfig = _.memoize(function findBindConfig(N, apiPath) {
  ...
  //
  ctx.url_to = function url_to_helper(apiPath, params, prefixOptions) {
    // Reconstruct prefixOptions to prevent side-effects.
    prefixOptions = _.pick(prefixOptions || {}, 'protocol', 'hostname', 'port');

    // Detect protocol.
    if (!prefixOptions.protocol) {
      prefixOptions.protocol = ctx.request.isEncrypted ? 'https' : 'http';
    }

    // Detect hostname.
    if (!prefixOptions.hostname && ctx.origin.req.headers['host']) {
      prefixOptions.hostname = ctx.origin.req.headers['host'].split(':')[0];
    }

    var bind = findBindConfig(N, apiPath);

    // Detect port.
    if (bind && !prefixOptions.port) {
      if ('https' === prefixOptions.protocol) {
        // For encrypted HTTPS connection.
        if (bind.ssl.forwarded || DEFAULT_HTTPS_PORT === bind.ssl.listen.port) {
          prefixOptions.port = null; // Do not set default port explicitly.
        } else {
          prefixOptions.port = bind.ssl.listen.port;
        }
      } else {
        ...
        if (bind.forwarded || DEFAULT_HTTP_PORT === bind.listen.port) {
          ...
          prefixOptions.port = bind.listen.port;
        }
      }
    }

    return N.runtime.router.linkTo(apiPath, params, prefixOptions);
  };

  // Client helper closures. Used by view templates.
  //
