...
//
helpers.set_layout = function set_layout_helper(layout) {
  this.res.layout = layout;
};

helpers.link_to = function link_to_helper(name, params) {
  return this.__N.router.linkTo(name, params) || '#';
};

// Constructs full URL using current env and N.config.bind
//
helpers.url_to = function url_to_helper(apiPath, params, linkDefaults) {
...
helpers.asset_path = function asset_path_helper(path) {
  var asset = this.__N.assets.manifest.assets[path];

  return asset ? this.__N.router.linkTo('core.assets', { path: asset }) : '#';
};

helpers.asset_include = function asset_include_helper(path) {
  ...
  if (asset) {
    try {
      ...
      this.__N.logger.error('Failed inline asset %s:\n%s'
      , path
...
helpers.date = function date_helper(value, format) {
  var locale = this.runtime.locale || this.__N.config.locales['default'];

  return date(value, format, locale, tzOffset);
};

helpers.add_raw_data = function add_raw_data_helper(key, data) {
...
function Env(N, options) {
  ...
  Object.keys(helpers).forEach(function (h) {
    this.helpers[h] = helpers[h].bind(this);
  }, this);
...
module.exports.helpers = helpers; // for modifications
