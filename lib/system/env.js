...
module.exports = function env(N, options) {
  ...
  var ctx = {
    ...
    request: {
      ...
      protocol: options.isEncrypted ? 'https' : 'http',
      hostname: req.headers.host ? req.headers.host.split(':')[0] : null,
      port:     req.headers.host ? req.headers.host.split(':')[1] : null
    },
  ...
  ctx.link_to = function link_to_helper(apiPath, params) {
    var prefixOptions = {
    ...
    // Detect protocol and port by current ApiPath.
    if (bind) {
      if (bind.force_ssl || ctx.request.isEncrypted) { // HTTP Secure
        prefixOptions.protocol = 'https';
  ...
  ctx.helpers.link_to = ctx.link_to;
