...
(function (root) {
  ...
  function WireHandler(channel, options, func) {
    if (channel.indexOf('*') !== -1 &&
      ...
      throw 'Bad channel name \'' + channel + '\'. Broadcast symbol (*) must ' +
  ...
  Wire.prototype.checkSkip = function (handlerName, channelName) {
    ...
    for (index = 0, length = skipList.length; index < length; index += 1) {
      ...
      if (skipBroadcast && (0 === channelName.indexOf(skip.slice(0, -1)))) {
        return true;
      }
    }
  ...
  Wire.prototype.getHandlers = function (channel) {
    ...
    if (!this.__sortedCache__[channel]) {
      ...
      this.__handlers__.forEach(function (handler) {
        ...
        if (handler.isBroadcast && (0 !== channel.indexOf(handler.lookupString))) {
          return;
        }
  ...
  function emitSingle(self, channel, params, callback) {
    ...
    function walk(err) {
      ...
      if (!wh.sync) {
        ...
        try {
          // Legacy style support - when error returned via result.
          // Sync function should return error via exception.
          _err = fn(params);
        } catch (e) {
          _err = e;
        }
  ...
  Wire.prototype.skip = function (channel, skipList) {
    ...
    skipList.forEach(function (name) {
      if (!self.__skips__.hasOwnProperty(name)) {
        self.__skips__[name] = [];
      }

      // Add channel name only if it not exists.
      if (-1 === self.__skips__[name].indexOf(channel)) {
        self.__skips__[name].push(channel);
      }
    });
