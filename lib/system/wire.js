...
(function (root) {
  ...
  Wire.prototype.checkSkip = function (handlerName, channelName) {
    ...
    for (index = 0, length = skipList.length; index < length; index += 1) {
      ...
      if (!skipBroadcast && skip === channelName) {
        return true;
      }

      if (skipBroadcast && skip.slice(0, -1) === channelName.slice(0, skip.length - 1)) {
        return true;
      }
    }
    ...
    if (!this.__sortedCache__[channel]) {
      ...
      this.__handlers__.forEach(function (handler) {
        ...
        if (!handler.isBroadcast &&
            handler.lookupString !== channel) {
          return;
        }

        // Broadcast handler - match as a prefix.
        if (handler.isBroadcast &&
            handler.lookupString !== channel.slice(0, handler.lookupString.length)) {
          return;
        }
