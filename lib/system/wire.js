...
Wire.prototype.getHandlers = function (channel) {
  if (!this.__sorted__[channel]) {
    this.__sorted__[channel] = _.chain(this.__handlers__).filter(function (wh) {
  ...
  return this.__sorted__[channel];
};
...
Wire.prototype.off = function (channel, handler) {
  this.__sorted__ = [];
  this.__handlers__ = _.filter(this.__handlers__, function (wh) {
    var match = wh.pattern.test(channel);

    if (handler) {
      match = match && wh.func === handler;
    }
...
Wire.prototype.has = function (channel, options, handler) {
  return _.any(this.__handlers__, function (wh) {
    var match = wh.pattern.test(channel);
    ...
    if (_.has(options, 'priority')) {
      match = match && wh.priority === options.priority;
    }

    if (_.has(options, 'ensure')) {
      match = match && !!wh.ensure;
    }
