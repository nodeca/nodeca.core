...
WireHandler.prototype.run = function (params, callback) {
  this.ncalled++;
...
function Wire() {
  prop(this, '__handlers__', {
    value: new Dict(null, function (key) {
      var val = new SortedArray([], WireHandler.equals, WireHandler.compare);
...
Wire.prototype.emit = function (channel, params, callback) {
  ...
  async.forEachSeries(this.__getHandlers(channel), function (handler, next) {
...
Wire.prototype.once = function (channel, options, handler) {
  ...
  prop(realHandler, '__original', handler);
...
Wire.prototype.off = function (channel, handler) {
  var arr = this.__handlers__.get(channel);
  ...
  arr.forEach(function (wh) {
    if (wh.func === handler || wh.func.__original === handler) {
      this["delete"](wh);
    }
  }, this);
};
