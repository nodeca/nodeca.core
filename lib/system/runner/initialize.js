...
function cleanupConfigs(cfg) {
  ...
  Object.values(cfg).forEach(cleanupConfigs);
}
...
function loadConfigs(root) {
  ...
  .forEach(file => {
    mergeConfigs(config, yaml.load(fs.readFileSync(file, 'utf8'), { filename: file }));
  });
...
function parseLoggerName(name) {
  ...
  if (!splittedMethod.some(Boolean)) {
    splittedMethod = null;
  }
...
function initWire(N) {
  ...
  function findPuncher(params) {
    // Try find puncher
    return params?.extras?.puncher ||
           params?.env?.extras?.puncher;
  }
...
function initScope(N) {
  ...
  N.validate = function (apiPath, schema) {
    ...
    if (!schema.properties.$query) {
      // allow querystring by default if its not already specified in schema
  ...
  /** internal
   *  N.validate.process(apiPath, rawParams) -> Object|Null
   *
   *  Validate `raw_params` against apiPath schema, any undeclared properties
   *  in `raw_params.$query` will be moved to `params.$query_extra`.
   *
   *  Runs revalidate of apiPath for given params. Returns an object:
   *    - is_valid (Boolean) - true if validation succeeded
   *    - errors   (Array)   - list of errors, each error is { field, message, value }
   *    - params   (Object)  - filtered params you should be using instead of raw_params
   *
   *  If apiPath has no schema, `Null` is returned instead.
   **/
  N.validate.process = function (apiPath, rawParams) {
    if (validateFn[apiPath]) {
      let filteredParams = _.cloneDeep(rawParams);

      if (validateFn[apiPath](filteredParams)) {
        // everything is validated correctly, no extra params in querystring
        return { valid: true, errors: [], params: rawParams };
      }

      // `filteredParams` is now the same as `rawParams` with undeclared
      // properties filtered out

      // if url doesn't have querystring, return validation errors as is
      if (!_.isPlainObject(filteredParams.$query)) {
        return { valid: false, errors: validateFn[apiPath].errors, params: rawParams };
      }

      // if url does have querystring, look at which properties were removed
      let extraQueryKeys = _.difference(Object.keys(rawParams.$query), Object.keys(filteredParams.$query));

      // if nothing was removed from querystring, return validation errors as is
      if (extraQueryKeys.length === 0) {
        return { valid: false, errors: validateFn[apiPath].errors, params: rawParams };
      }

      // create a separate list of undeclared querystring arguments for future use
      let newParams = _.cloneDeep(rawParams);
      let extraQueryParams = Object.create(null);

      // move undeclared querystring arguments from rawParams
      for (let key of extraQueryKeys) {
        extraQueryParams[key] = newParams.$query[key];
        delete newParams.$query[key];
      }

      // re-run validation on original params without undeclared querystring params
      let result = validateFn[apiPath](newParams);

      // keep undeclared querystring params for future use
      newParams.$query_extra = extraQueryParams;

      if (result) {
        return { valid: true, errors: [], params: newParams };
      }

      return { valid: false, errors: validateFn[apiPath].errors, params: newParams };
    }
...
// Finalize config, expand environment-dependent configs
//
function initConfig(N) {
  ...
  for (let [ key, val ] of Object.entries(N.config)) {
    if (key[0] === '^') {
      delete N.config[key];
...
function initLogger(N) {
  let mainRoot     = N.mainApp.root,
      config       = Object.assign({}, N.config.logger),
      options      = Object.assign({ file: { logSize: 10, backups: 5 } }, config.options),
  ...
  N.logger.getLogger = function (name) {
    ...
    //
    let chosenLogger = loggers.find(logger => {
  ...
  for (let [ name, loggerConfig ] of Object.entries(config)) {
    let appendersInGroup = [];
    let groupLevel = log4js.levels.FATAL;

    for (let appenderConfig of loggerConfig) {
      ...
      if (!log4js_options.appenders['file:' + appenderConfig.file] && cluster.isMaster) {
        ...
        let appender = {
          type:        options.file.daysToKeep ? 'dateFile' : 'file',
          filename,
          layout:      plainLayout,
          keepFileExt: true
        };
        ...
        if (options.file.logSize) {
          appender.maxLogSize = options.file.logSize * 1024 * 1024;
          appender.backups    = options.file.backups;
        }

        if (options.file.daysToKeep) {
          appender.daysToKeep = options.file.daysToKeep;
        }

        log4js_options.appenders['file:' + appenderConfig.file] = appender;
      }
    ...
    if (appendersInGroup.length) {
      /* eslint-disable max-depth */
      for (let appender of appendersInGroup) {
...
// Load app and all nested apps, merge their configuration
//
function loadApp(N, appPath) {
  let app = new Application(require(appPath));
  let appConfigPath = path.join(app.root, 'config');

  N.apps.push(app);
  mergeConfigs(N.config, loadConfigs(appConfigPath));

  for (let m of app.config.modules) {
    loadApp(N, path.join(app.root, m));
  }
}


// Load configuration for all enabled apps.
//
function loadApps(N, applications) {
  N.apps = [ N.mainApp ];

  // Try load each enabled application and push to the array of loaded apps
  for (let appDef of (applications || [])) loadApp(N, getAppName(appDef));
}
...
module.exports = async function (N) {
  ...
  // Create empty object that will be filled by each app later
  N.config = {};

  loadApps(N, mainConfig.applications);

  // merge in main config
  mergeConfigs(N.config, mainConfig);

  // resolve per-environment configs
  initConfig(N);
  ...
  for (let app of N.apps) app.init(N);
  ...
  for (let app of N.apps) {
    // `package.json` is not mandatory (likely won't exist for submodules)
    try {
      hasher.update(fs.readFileSync(path.join(app.root, 'package.json'), 'utf-8'));
    } catch (__) {}

    // `yarn.lock` is not mandatory (likely won't exist for submodules)
    try {
      hasher.update(fs.readFileSync(path.join(app.root, 'yarn.lock'), 'utf-8'));
    } catch (__) {}
