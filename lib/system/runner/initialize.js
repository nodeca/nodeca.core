...
function initLogger(N) {
  ...
  function clusteredAppender(children, category) {
    ...
    let appender = log4js.appenders.clustered(clusterAppenderConfig);
    let fallback = log4js.appenders.console();

    return logEvent => {
      if (cluster.isWorker && process.connected === false) {
        // we can't log this because worker is disconnected from IPC
        // (means, it's currently shutting down)
        //
        // if `logToConsole` is true, it's already logged
        if (!logToConsole) { fallback(logEvent); }
      } else {
        appender(logEvent);
      }
    };
