...
function mergeConfigs(dst, src) {
  _.each(src || {}, function (value, key) {
    ...
    // destination not exists -> create it
    if (!_.has(dst,key)) {
      dst[key] = value;
      return;
    }
    ...
    // Other types can not be merged, leave intact
    // Since root application config is loaded first, that gives
    // us desired priority - root config is more significant
    return;
  });
...
module.exports = function (N, callback) {
  ...
  async.series([initConfig, initLogger, checkConfig, initApps], callback);
};
