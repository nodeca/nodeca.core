...
//  common.render(apiPath[, locals]]) -> String
//  - templates (Object): Templates collection. Normally it should be`N.views`.
...
//
module.exports = function render(templates, apiPath, locals) {
  var templateFn
    , translateFn
    , result = '';

  if (templates[apiPath]) {
    templateFn = templates[apiPath];
  } else {
  ...
  locals = _.defaults(locals || {}, {
    partial: function (partialPath) {
      return render(templates, resolveApiPath(partialPath, locals), locals);
    }
