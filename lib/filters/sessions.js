...
var COOKIE_KEY  = 'nodeca.sid'; // Cookie ID
...
function setSession(sid, sess, callback) {
  ...
  nodeca.runtime.redis.setex(PREFIX + sid, TTL, sess, callback);
}
...
//
nodeca.filters.before('', { weight: -8000 }, function sessions(params, callback) {
  ...
  //
  // Set or remove COOKIE with session ID
  //

  res.on('header', function () {
    res.setHeader('Set-Cookie', cookie.serialize(COOKIE_KEY, env.session ? sid : null, {
      maxAge: SESSION_TTL
    }));
  });
