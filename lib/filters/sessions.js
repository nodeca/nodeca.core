...
function initSession(env) {
  ...
  // prefill session
  env.session = _.extend(env.session || {}, {
...
function setSession(sid, sess) {
  if (!sess) {
    nodeca.runtime.redis.del(PREFIX + sid);
    return;
  }

  nodeca.runtime.redis.setex(PREFIX + sid, SESSION_TTL, JSON.stringify(sess));
}
...
//
nodeca.filters.ensure('', { weight: 9000 }, function save_session(params, callback) {
