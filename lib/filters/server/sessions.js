...
function getSession(sid, callback) {
  N.components.redis.get(PREFIX + sid, function (err, data) {
    if (err) {
      callback(err);
      return;
    }
...
function setSession(sid, sess) {
  if (!sess) {
    N.components.redis.del(PREFIX + sid);
    return;
  }

  N.components.redis.setex(PREFIX + sid, SESSION_TTL, JSON.stringify(sess));
}
...
//
N.filters.before('', { weight: -80 }, function load_session(params, callback) {
...
//
N.filters.ensure('', { weight: 90 }, function save_session(params, callback) {
