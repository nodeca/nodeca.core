...
function sortLanguages(a, b) {
  var aq = a.quality || 1, bq = b.quality;
...
//
function findAcceptedLanguage(origin) {
  var locales, l, req = origin && origin.req;
...
nodeca.filters.before('', { weight: -65 }, function preset_locale(params, callback) {
  ...
  if (!locale || !enabledLocales[locale]) {
    ...
    locale = findAcceptedLanguage(this.origin.http || this.origin.rpc);
  }
  ...
  if (this.session && this.session.locale !== this.runtime.locale) {
