...
nodeca.filters.before('', { weight: -65 }, function preset_locale(params, callback) {
  var locale, req = (this.origin.http || this.origin.rpc).req;
  ...
  if (!locale || !enabledLocales[locale]) {
    ...
    locale = findAcceptedLanguage(req);
  }
  ...
  // save current locale in the request object for error reporting
  req.locale = this.runtime.locale;
