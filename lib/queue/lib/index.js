...
Queue.prototype.__retryMapping__ = function (task_id_global, timestamp) {
  ...
  self.__redis__.multi()
    ...
    // If another process finish and delete task - skip
    if (!data[2]) {
      return;
    }
  ...
  self.__redis__.time(function (err, time) {
    ...
    self.__redis__.eval(self.__scripts__.chunk_pending_to_active, 1, task_id_global, deadline, function (err, chunkID) {
      ...
      self.__redis__.multi()
        ...
        // If another process finish and delete chunk - skip
        if (!data[1]) {
          return;
        }
...
// Get all tasks for worker by name
//
// - workerName (String)
//
Queue.prototype.__getTasks__ = function (worker, callback) {
  var self = this;

  // Get all taskIDs
  this.__redis__.multi()
    ...
    taskIDs = taskIDs.filter(function (taskID) {
      return _.startsWith(taskID, self.__prefix__ + worker.name + ':');
    });
