...
//
Queue.prototype.status = function (taskID, callback) {
  var self = this;
  var result = {};

  self.__redis__.hmget(taskID, 'type', 'state', function (err, data) {
    if (err) {
      callback(err);
      return;
    }
    ...
    self.__redis__.multi()
        .lrange(taskID + ':chunks:pending', 0, -1)
...
// - callback (Function) - called as: `function (err)`
//
Queue.prototype.cancel = function (taskID, callback) {
  this.__deleteTask__(taskID, callback);
};
...
Queue.prototype.__retryMapping__ = function (taskID, timestamp) {
  ...
  self.__redis__.multi()
    ...
    if (!worker) {
      self.emit('error', new QueueError('Broken task type.', workerName, 'mapping', taskID, null, taskData));
      return;
    }
...
Queue.prototype.__consumeTask__ = function (taskID) {
  ...
  self.__redis__.multi()
    ...
    // If task type is broken. This should never happens because we check worker before this method call.
    if (!worker) {
      self.emit('error', new QueueError('Broken task type.', workerName, 'pending', taskID, null, taskData));
      return;
    }
...
Queue.prototype.__doProcess__ = function (chunk, deadline) {
  ...
  self.__redis__.time(function (err, now) {
    ...
    // If chunk will suspend or throw unhandled exception we should decrement
    // tracker value by timeout.
    ...
    chunk.process(function (err, result) {
      ...
      // The rest of chunk state will be managed by watchdog
      if (terminated) {
        if (err) {
...
// - taskID (String) - the task ID
//
Queue.prototype.__consumeChunk__ = function (worker, taskID) {
  var self = this;
  ...
  self.__redis__.time(function (err, time) {
    ...
    // - deadline timestamp
    //
    self.__redis__.eval(self.__scripts__.chunk_pending_to_active, 1, taskID, deadline, function (err, chunkID) {
      if (err) {
        self.emit('error', err);
        return;
      }
...
Queue.prototype.__retryReducing__ = function (taskID, timestamp) {
  ...
  self.__redis__.multi()
    ...
    // If task type is broken. This should never happens because we check worker before this method call.
    if (!worker) {
      self.emit('error', new QueueError('Broken task type.', workerName, 'reducing', taskID, null, null));
      return;
    }
...
Queue.prototype.__deleteChunk__ = function (taskID, chunkID) {
  ...
  self.__redis__.multi()
      .lrem(taskID + ':chunks:pending', 0, chunkID)
...
//
Queue.prototype.__deleteTask__ = function (taskID, callback) {
  var self = this;
...
Queue.prototype.__doAggregate__ = function (taskID) {
  ...
  self.__redis__.multi()
    ...
    // If task type is broken. This should never happens because we check worker before this method call.
    if (!worker) {
      self.emit('error', new QueueError('Broken task type.', workerName, 'aggregating', taskID, null, null));
      return;
    }
    ...
    self.__redis__.eval(
      ...
      function (err) {
        ...
        self.__redis__.llen(taskID + ':chunks:pending', function (err, countAvailable) {
          if (err) {
            self.emit('error', err);
            return;
          }
          ...
          //
          var cnt = Math.min(worker.chunksPerInstance - worker.chunksTracker[taskID], countAvailable);

          for (var i = 0; i < cnt; i++) {
            self.__consumeChunk__(worker, taskID);
          }
        });
      }
    );
