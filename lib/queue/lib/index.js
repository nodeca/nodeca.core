...
//   - cron (String) - optional, cron string ("15 */6 * * *"), default null
//   - noTrack (Boolean) - By default queue tracks all scheduled tasks, to avoid rerun if several servers in
//     cluster have wrong clocks. Locks list is stored for 3 days. But that can take a lot of memory, if task
//     is scheduled too often (ping every second, for example). Set `noTrack` = true to disable such tracking.
...
Queue.prototype.__schedule__ = function (worker) {
  ...
  var job = new CronJob(worker.cron, function scheduledRun() {
    ...
    if (worker.noTrack) {
      self.__addTask__(worker, null, function (err) {
        if (err) {
          self.emit('error', err);
        }
      });
...
// - callback (Function)
//
Queue.prototype.__addTask__ = function (worker, taskData, callback) {
  var self = this;
...
//
Queue.prototype.__retryMapping__ = function (task_id_global, timestamp, callback) {
  var self = this;
...
// - task_id_global (String) - the task ID
//
Queue.prototype.__consumeTask__ = function (task_id_global, callback) {
  var self = this;
...
Queue.prototype.__doReduce__ = function (task, timestamp) {
  ...
  self.__redis__.smembers(task_id_global + ':chunks:done', function (err, chunksIDs) {
    if (err) {
      self.emit('error', err);
      return;
    }
...
// - timestamp (Number) - old task's deadline. Used to avoid races on retrying task by watchdog
//
Queue.prototype.__retryReducing__ = function (task_id_global, timestamp, callback) {
  var self = this;
...
// - task_id_global (String) - the task ID
//
Queue.prototype.__doAggregate__ = function (task_id_global, callback) {
  var self = this;
...
// - callback (Function)
//
Queue.prototype.__setTaskDeadline__ = function (task, timeLeft, callback) {
  var self = this;
...
// - callback (Function)
//
Queue.prototype.__setChunkDeadline__ = function (chunk, timeLeft, callback) {
  var self = this;
  ...
  self.__redis__.time(function (err, redisTime) {
    ...
    self.__redis__.zadd(task_id_global + ':chunks:active', 'XX', deadline, chunk.id, callback);
  });
};
...
// - workerName (String) - the worker name
//
Queue.prototype.__cleanupWorker__ = function (workerName, callback) {
  var self = this;
  ...
  self.__redis__.multi()
    ...
    // Remove tasks
    async.each(taskIDs, function (taskID, cb) {
      self.__deleteTask__(taskID, cb);
    }, function (err) {
      if (err) {
        self.emit('error', err);
        callback();
      ...
      // Remove worker
      self.__redis__.zrem(self.__prefix__ + 'workers', workerName, function (err) {
        if (err) {
          self.emit('error', err);
        }
...
Queue.prototype.__tick__ = function () {
  ...
  self.__redis__.time(function (err, redisTime) {
    ...
    async.parallel([
      ...
      //
      function (next) {
        self.__redis__.zrangebyscore(self.__prefix__ + 'postponed', '-inf', time, function (err, taskIDs) {
          if (err) {
            next();
            return;
          }
      ...
      function (next) {
        self.__redis__.smembers(self.__prefix__ + 'pending', function (err, taskIDs) {
          ...
          async.each(taskIDs, function (taskID, cb) {
            self.__consumeTask__(taskID, cb);
          }, next);
        });
      },
      ...
      function (next) {
        self.__redis__.smembers(self.__prefix__ + 'aggregating', function (err, taskIDs) {
          ...
          async.each(taskIDs, function (taskID, cb) {
            self.__doAggregate__(taskID, cb);
          }, next);
        });
      },
      ...
      function (next) {
        ...
        self.__redis__.zrangebyscore(self.__prefix__ + 'mapping', '-inf', time, 'withscores', function (err, result) {
          if (err) {
            next();
            return;
          }
          ...
          async.each(items, function (item, cb) {
            self.__retryMapping__(item.taskID, item.score, cb);
          }, next);
        });
      },
      ...
      function (next) {
        // Get tasks IDs and its deadlines
        self.__redis__.zrangebyscore(self.__prefix__ + 'reducing', '-inf', time, 'withscores', function (err, result) {
          if (err) {
            next();
            return;
          }
          ...
          async.each(items, function (item, cb) {
            self.__retryReducing__(item.taskID, item.score, cb);
          }, next);
        });
      },
      ...
      function (next) {
        ...
        self.__redis__.zadd(self.__prefix__ + 'workers', args, function (err) {
          if (err) {
            self.emit('error', err);
          }
      ...
      function (next) {
        ...
        self.__redis__.zrangebyscore(self.__prefix__ + 'workers', '-inf', time - WORKER_TTL, function (err, workers) {
          if (err) {
            next();
            return;
          }

          async.each(workers, function (workerName, cb) {
            self.__cleanupWorker__(workerName, cb);
          }, next);
        });
      }
