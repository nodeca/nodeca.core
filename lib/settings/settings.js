 ...
 **/
Settings.prototype.get = Promise.coroutine(function* get(keys, params, options) {
  let results     = {};
  ...
  (yield Promise.map(Object.keys(store_keys), name => this.getStore(name).get(store_keys[name], params, options)))
    .forEach(data => {
...
Settings.prototype.getByCategory = function (categoryKey, params, options) {
  let keys = _.reduce(
    this.__stores__,
    (result, store) => result.concat(store.keysByCategory(categoryKey)),
    []);
  ...
  return this.get(keys, params, options);
};
