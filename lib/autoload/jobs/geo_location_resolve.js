  ...
  async function resolve_location_overpass(lat, lon, locale) {
    ...
    if (cached) {
      ...
      try {
        ...
        if ((err.response || {}).statusCode !== 400) throw err;
      ...
      await N.models.core.GeoOverpass.updateOne(
        ...
        { $set: _.omitBy({
          ...
          error_code: error ? ((error.response || {}).statusCode || error.code) : null,
          ts:         new Date()
        }, _.isNil) },
        { upsert: true }
      );
    ...
    N.queue.registerTask({
      ...
      async process() {
        ...
        for (;;) {
          ...
          let params = await N.redis.lpop(queue_name);

          if (!params) {
            queue_name = 'geo:location';
            params = await N.redis.lpop(queue_name);
          }
          ...
          try {
            ...
            await N.redis.rpush(queue_name, params);
