...
module.exports = function (N) {
  ...
  const resolve_location_overpass = Promise.coroutine(function* (lat, lon, locale) {
    let provider_name = 'overpass';

    let req_id = N.models.core.GeoOverpass.hash([ lon, lat ]);

    let cached = yield N.models.core.GeoOverpass.findOne({ hash: req_id }).lean(true);
    let body;
    ...
    if (cached) {
      ...
      try {
        response = yield got('http://overpass-api.de/api/interpreter', {
          query:   { data: script },
      ...
      yield N.models.core.GeoOverpass.update(
        { hash: req_id },
        { $set: _.omitBy({
          location:   [ lon, lat ],
  ...
  //
  const resolve_location_overpass_nearest = Promise.coroutine(function* (lat, lon, locale) {
    let provider_name = 'overpass_nearest';

    let req_id = N.models.core.GeoOverpassNearest.hash([ lon, lat ]);

    let cached = yield N.models.core.GeoOverpassNearest.findOne({ hash: req_id }).lean(true);
    let body;
    ...
    if (cached) {
      ...
      yield N.models.core.GeoOverpassNearest.update(
        { hash: req_id },
        { $set: _.omitBy({
          location:   [ lon, lat ],
    ...
    if (elements[0] && elements[0].tags) {
      ...
      if (name) {
        return { name: '~' + name, distance: elements[0].distance, provider: provider_name };
      }
    }
  ...
  const resolve_location_nominatim = Promise.coroutine(function* (lat, lon, locale) {
    let provider_name = 'nominatim';

    let req_id = N.models.core.GeoNominatim.hash([ lon, lat ], locale);

    let cached = yield N.models.core.GeoNominatim.findOne({ hash: req_id }).lean(true);
    let body;
    ...
    if (cached) {
      ...
      yield N.models.core.GeoNominatim.update(
        { hash: req_id },
        { $set: _.omitBy({
          location:   [ lon, lat ],
    ...
    N.queue.registerTask({
      ...
      process: Promise.coroutine(function* () {
        ...
        for (;;) {
          ...
          try {
            result = yield resolve_location_overpass(lat, lon, locale);
            if (!result) result = yield resolve_location_nominatim(lat, lon, locale);
            if (!result) result = yield resolve_location_overpass_nearest(lat, lon, locale);
          } catch (err) {
            // in case of errors, put coordinates back into queue and
            // pause this task for 10 minutes
            N.logger.error('Geocoding service error: ' + (err.message || err));
            yield N.redis.rpushAsync(queue_name, params);
            this.setDeadline(12 * 60 * 1000);
            yield Promise.delay(10 * 60 * 1000);
            continue;
          }
