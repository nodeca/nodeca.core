  ...
  N.wire.on('init:jobs', function register_expand_links_cache() {
    ...
    N.queue.registerTask({
      ...
      async process() {
        ...
        let uid = this.uid;
        let current = 0;
        let total = await N.models.core.UrlTracker.countDocuments()
        ...
        async function get_url_block() {
          ...
          if (chunk.length === 0) {
            ...
            // stop the task if no urls are being fetched and no further chunks are found
            // (happens for example if UrlTracker collection is empty)
            if (pending <= 0) finish_task();
