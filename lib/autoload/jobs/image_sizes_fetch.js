  ...
  N.wire.on('init:jobs', function register_image_sizes_fetch() {
    ...
    N.queue.registerTask({
      ...
      async process() {
        ...
        let uid = this.uid;
        let current = 0;
        let total = await N.models.core.ImageSizeCache.count()
        ...
        let errors_by_domain = Object.create(null);
        ...
        relimit = new Relimit({
          ...
          async process(item) {
            ...
            try {
              if (errors_by_domain[domain] >= BLACKLIST_AFTER_FAILS) {
                let e = new Error('ETIMEDOUT');
                e.code = 'ETIMEDOUT';
                throw e;
              }

              result = await get_size(url);
              delete errors_by_domain[domain];
            } catch (_err) {
              err = _err;
            }
            ...
            if (err) {
              ...
              if (err.code === 'ETIMEDOUT') {
                errors_by_domain[domain] = (errors_by_domain[domain] || 0) + 1;

                if (errors_by_domain[domain] === BLACKLIST_AFTER_FAILS) {
                  N.logger.error(`${task_name}: domain ${domain} blacklisted, too many timeouts`);
                }
              }
