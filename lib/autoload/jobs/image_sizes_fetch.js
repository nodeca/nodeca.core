  ...
  N.wire.on('init:jobs', function register_image_sizes_fetch() {
    ...
    N.queue.registerTask({
      ...
      async init() {
        ...
        let url_count = await N.models.core.ImageSizeCache.count();
        ...
        tasks.push(N.queue.image_sizes_fetch_reduce());
    ...
    N.queue.registerTask({
      ...
      async process(from, to, runid) {
        ...
        await Promise.map(url_sizes, async function (u) {
          extendDeadline();
          ...
          try {
            result = await get_size(url, { headers: { 'user-agent': userAgent } });
          } catch (_err) {
            err = _err;
          }
        ...
        let task = await N.queue.getTask('image_sizes_fetch');
