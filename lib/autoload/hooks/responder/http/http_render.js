...
const render = require('../../../../system/render/common');
...
module.exports = function (N) {
  ...
  N.wire.after('responder:http', { priority: 10 }, function http_render(env) {
    ...
    let inject_headers = []
      .concat(N.config.options?.inject_headers || [])
      .concat(env.inject_headers || []);

    let inject_body_start = []
      .concat(N.config.options?.inject_body_start || [])
      .concat(env.inject_body_start || []);
    ...
    try {
      env.body = render(N, view, res, Object.assign({
        runtime: env.runtime,
        apiPath: env.method
      }, env.helpers));
      ...
      if (env.res.layout) {
        ...
        env.body = render(N, env.res.layout, res, Object.assign({
          runtime: env.runtime,
          apiPath: env.method,
          content: env.body
