...
module.exports = function (N) {
  ...
  N.wire.on('init:services', { parallel: true }, function* worker_pool_queue_init(N) {
    ...
    if (!cluster.isMaster || fork < 1) {
      // single process mode
      yield N.wire.emit('init:services.worker-queue', N);
      return;
    }

    let pool = new WorkerPool('worker-queue', fork);
    ...
    pool.on('worker:online', co.wrap(function* (pid) {
      try {
  ...
  N.wire.after('init:models', function queue_init(N) {

    var lossy_logger = _.throttle(function (err) {
      N.logger.error(err);
    }, 10000);

    N.queue = new Queue(N.config.database.redis);
    ...
    N.wire.on('init:services.worker-queue', function queue_start() {
      N.queue.start();
    });
    ...
    N.wire.once('exit.shutdown', { ensure: true, parallel: true }, function close_queue() {
      ...
      if (N.queue.idle) return;

      return new Promise(resolve => {
        N.queue.once('idle', resolve);
        setTimeout(() => resolve(), 10000);
      });
    });

    return N.wire.emit('init:jobs', N);
  });
};
