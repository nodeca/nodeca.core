...
const env         = require('nodeca.core/lib/system/env');
...
module.exports = function (N) {
  ...
  // `init:server.worker-http` instead
  //
  let active_pools = [];

  N.wire.on('init:services', { parallel: true }, function* worker_pool_http_init(N) {
    ...
    if (!cluster.isMaster || fork < 1) {
      // single process mode
      yield N.wire.emit('init:services.worker-http', N);
      return;
    }

    let pool = new WorkerPool('worker-http', fork);
    ...
    N.wire.on('exit.shutdown', { ensure: true, parallel: true }, function shutdown_http_pool(__, callback) {
      pool.once('exit', callback);
      pool.shutdown();
    });

    N.wire.on('exit.terminate', { ensure: true, parallel: true }, function terminate_http_pool(__, callback) {
      pool.once('exit', callback);
      pool.terminate();
    });
  ...
  N.wire.on('init:services.worker-http', function* server_bind(N) {
