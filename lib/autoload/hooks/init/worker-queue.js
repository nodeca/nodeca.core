...
module.exports = function (N) {
  ...
  N.wire.after('init:models', function queue_init(N) {
    ...
    N.wire.on('init:server.worker-queue', function queue_start() {
      N.queue.start();
    });

    N.wire.once('exit.shutdown', { ensure: true }, function close_queue() {
      N.queue.shutdown();
      ...
      N.wire.on('exit', { ensure: true }, function wait_for_queue_close() {
        return wait_for;
      });
    });
