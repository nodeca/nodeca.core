...
module.exports = function (N) {
  ...
  N.wire.before('init:bundle', function init_date_l10n_cldr() {
    let cldr = _.merge(
      {},
      require('cldr-core/availableLocales'),
      require('cldr-core/supplemental/parentLocales')
    );

    let cldr_locales = {},
        cldr_locales_names;

    cldr.availableLocales.modern.forEach(l => { cldr_locales[l] = l; });
    ...
    N.config.locales.forEach(locale => {
      ...
      let data = require('cldr-dates-modern/main/' + cldr_locales[name] + '/ca-gregorian')
