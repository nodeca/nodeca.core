...
Migration.statics.markPassed = function (app_name, step, callback) {
  ...
  self.find({ _id: app_name }, function (err, docs) {
    if (err) {
      callback(err);
      return;
    }
    ...
    if (docs.length > 0) {
      self.update({ _id: app_name }, { $push: { steps: step } }, callback);
    } else {
      self.create({ _id: app_name, steps: [ step ] }, callback);
    }
  });
};
  ...
  N.wire.after('init:models', { priority: 999 }, function migrations_check(N, next) {
    ...
    N.models.Migration.getLastState(function (err, currentMigrations) {
      ...
      if (N.models.Migration.checkMigrations(N, currentMigrations).length > 0) {
        next('Can\'t start: database changed. Please, run `migrate --all` command');
        return;
      }
