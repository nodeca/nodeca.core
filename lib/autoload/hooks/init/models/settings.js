...
module.exports = function (N) {
  ...
  //
  N.wire.after('init:models', { priority: 50 }, function* emit_init_settings() {
    N.settings = new Settings();

    yield N.wire.emit('init:settings', {});
  });
  ...
  N.wire.on('init:settings', function normalize_setting_schemas() {
    ...
    _.forEach(N.config.setting_schemas, store => {
      _.forEach(store, (schema, name) => {
    ...
    _.forEach(schemasCache, (schemaList, settingName) => {
      let baseSchema = _.find(schemaList, schema => !schema.extends);
  ...
  //
  N.wire.on('init:settings', function* resolve_schema_defaults() {
    let resolvers = [];
      ...
      _.forEach(store, schema => {
        ...
        resolvers.push(schema.values().then(values => {
          ...
          function findValue(configured) {
            let fetched = _.find(values, { name: configured });
  ...
  N.wire.on('init:settings', function normalize_setting_groups() {
    ...
    _.forEach(N.config.setting_groups, (group, name, collection) => {
  ...
  N.wire.on('init:settings', function init_settings() {
    N.apps.forEach(app => {
      ...
      .forEach(name => {
        ...
        _.forEach(N.config.setting_schemas[apiPath], (schema, key) => {
