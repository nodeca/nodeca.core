  ...
  N.wire.before('init:models', function redis_init(N) {
    ...
    return new Promise((resolve, reject) => {
      ...
      N.redis.once('connect', function () {
        ...
        // select used database, we only need to do it once because
        // redis stores and selects it automatically on reconnect
        var parsed = redisUrl.parse(url);

        // flag forces to schedule the command even if redis is not ready yet
        N.redis.send_anyway = true;
        N.redis.select(parsed.database, () => resolve());
        N.redis.send_anyway = false;
      });
    });
  });
};
