...
module.exports = function (N) {
  ...
  //
  const expand_links = Promise.coroutine(function* _expand_links(data, is_preview) {
    let items = data.ast.find('msg-link').slice(0, LIMIT);
    ...
    if (urls.length) {
      ...
      urls.forEach(url => {
        bulk.find({ url }).upsert().update({
          $setOnInsert: {
            url,
            status: N.models.core.UrlTracker.statuses.SUCCESS,
            rand: Math.random(),
            retries: 0,
            uses_unshort: false,
            uses_embedza: false
