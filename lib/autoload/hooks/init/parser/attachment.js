...
module.exports = function (N) {
  ...
  function fetch_attachments_meta(data, refs, callback) {
    ...
    refs.forEach(function (ref) {
      ...
      var attach_params = data.result.image_info[key] || (data.params.image_info || {})[key];
    ...
    var attach_list = {
      ids: _.sortedUniq(unknown_refs.map(function (r) { return r[0]; }).sort())
    };
    ...
    N.wire.emit('internal:common.content.attachments', attach_list, function (err) {
      ...
      refs.forEach(function (ref) {
        ...
        data.result.image_info[key] = _.assign(_.omitBy({
          type:      result[id].type,
        ...
        }, _.isUndefined), data.result.image_info[key], (data.params.image_info || {})[key]);
      });
