...
// - `N.live.debounce('channel.name', { foo: bar });` - filer messages via deboncer (3 sec) and send then
//
  ...
  function debounce(channel, data) {
    N.redis.time(function (err, time) {
      ...
      N.redis.multi()
          ...
          // Emulate ZADD NX behaviour. If last send ts does not exists - create
          // it. Init with -1, to avoid GC conflict (GC clears 0+ interval).
          .eval(`
            if redis.call("zrank", "live_debounce_last", KEYS[1]) == false then
              return redis.call("zadd", "live_debounce_last", -1, KEYS[1])
            end`, 1, channel)
