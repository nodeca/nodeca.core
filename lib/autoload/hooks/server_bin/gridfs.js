...
module.exports = function (N) {
  ...
  function isCacheOk(headers, fileInfo) {
    ...
    let noneMatch     = headers['if-none-match'];
    let cc            = headers['cache-control'];

    let lastModified  = fileInfo.uploadDate.getTime();
    let etag          = fileInfo.md5;
    ...
    // if-none-match
    if (noneMatch) { etagMatches = (noneMatch.indexOf(etag) !== -1) || noneMatch[0] === '*'; }

    // if-modified-since
    if (modifiedSince) {
      // Drop milliseconds, because header don't have those. When server
      // responded previous time, timestamp lost milliseconds.
      lastModified -= lastModified % 1000;

      notModified = lastModified <= (new Date(modifiedSince)).getTime();
    }
