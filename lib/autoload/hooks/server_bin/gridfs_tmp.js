...
module.exports = function (N) {
  ...
  N.wire.on('server_bin:core.gridfs_tmp', function* static_file_pre_send(env) {
    if ([ 'GET', 'HEAD' ].indexOf(env.origin.req.method) === -1) throw 403;

    env.data.fileInfo = yield N.models.core.FileTmp.getInfo(env.params.bucket);

    if (!env.data.fileInfo) throw 404;
  });


  N.wire.on('server_bin:core.gridfs_tmp', function static_file_send(env) {
    ...
    env.status = 200;
    // TODO: that will cause improper size logging on connection termination
    env.log_request(env);
    res.statusCode = env.status;
    ...
    pump(N.models.core.FileTmp.createReadStream(env.params.bucket), res, err => {
      if (err) N.logger.error(`Error while streaming ${env.params.bucket} from gridfs: ${err.message}`);
    });
  });
};
