...
module.exports = function (N) {
  ...
  N.wire.before('server_chain:http', { priority: -50 }, function csrf_set_token(env, callback) {
  ...
  N.wire.before('server_chain:rpc', { priority: -50 }, function csrf_check_token(env, callback) {
    // No session - skip CSRF protection.
    if (!env.session) {
      callback();
      return;
