...
module.exports = function (N) {
  ...
  function getAcceptedLanguages(string) {
    var result = [], fallbacks = {}, tmp;

    tmp = String(string).split(',').slice(0, 10).map(parseLanguagePart);

    tmp = _.sortBy(tmp, 'quality')
              .map(function (val) { return val.locale; })
              .reverse();

    tmp.forEach(function (locale) {
      ...
      if (fallback !== locale && !fallbacks[fallback] && tmp.indexOf(fallback) < 0) {
