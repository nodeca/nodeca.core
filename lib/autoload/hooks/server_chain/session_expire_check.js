...
module.exports = function (N) {
  N.wire.before('server_chain:*', { priority: -75 }, function session_expire_check(env, callback) {
    if (!env.session) {
      // No session.
      callback();
      return;
  ...
  N.wire.before('server_chain:*', { priority: -65 }, function session_create_time_set(env) {
    if (!env.session) {
      // No session.
      return;
    }
