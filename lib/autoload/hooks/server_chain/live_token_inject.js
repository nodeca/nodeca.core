...
module.exports = function (N) {
  ...
  //
  N.wire.after([ 'server_chain:rpc:*', 'server_chain:http:*' ], { priority: 70 }, function token_create(env, callback) {
    // If no session - skip
    if (!env.session) {
      callback();
      return;
  ...
  //
  N.wire.after([ 'server_chain:rpc:*', 'server_chain:http:*' ], { priority: 90 }, function token_store(env, callback) {
    // If no session - skip
    if (!env.session) {
      callback();
      return;
