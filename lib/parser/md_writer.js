...
const linkify = require('linkify-it')({ fuzzyLink: false });
...
class MarkdownWriter {
  ...
  tag_handler_anchor(el) {
    ...
    if ($el.children().length === 0 && $el.text() === dest && dest && !title) {
      return this.format_linkify(dest) ??
             this.format_autolink(dest) ??
             this.format_link(dest, dest);
    }
  ...
  // returns true if this link can be picked up by linkify rule from plain text
  can_be_linkify(dest) {
  ...
  format_linkify(dest) {
    return this.can_be_linkify(dest) ? dest : null;
  }

  // returns true if this link can be included in a valid <...> sequence
  can_be_autolink(dest) {
    return /^[a-z][a-z0-9+.\-]{1,31}:/i.test(dest);
  }

  format_autolink(dest) {
    return this.can_be_autolink(dest) ?
           // eslint-disable-next-line no-control-regex
           ('<' + dest.replace(/[<>\x00-\x20\x7f]/g, encodeURI) + '>') :
           null;
  }
