...
function get_text_length(node) {
  ...
  node.contents().each(function () {
    if (this.type === 'text') {
      length += charcount(this.data.replace(/\s+/g, ''));
...
function Parser() {
  var bus = this.bus = wire(wireOptions());
...
Parser.prototype.parse = function (params, callback) {
  ...
  if (!callback) {
    return this.bus.emit('parse', data).then(() => data.result);
  }
...
module.exports = function (params, callback) {
  return create(params.options).parse(params, callback);
};
...
//
module.exports.addPlugin = function (pluginName, plugin, persistent) {
