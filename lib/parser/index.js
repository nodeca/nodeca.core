...
//
function removeTags(node, whitelist) {
  node.children().each(function () {
    let $element = $(this);

    removeTags($element, whitelist);

    // whitelist all tags that we want to keep
    if (!$element.filter(whitelist).length) {
...
function Parser() {
  ...
  this.bus.on('md2html', function md2html_emit_render(data) {
    ...
    return bus.emit('md2html.render', data);
  });
  ...
  this.bus.on('html2preview', function html2preview_emit_render(data) {
    // Transform AST to preview format and render
    return bus.emit('html2preview.render', data);
  });

  this.bus.after('html2preview', function html2preview_serialize_ast(data) {
    removeTags(data.ast, data.whitelist.join(', '));
    getTextLength(data.ast, data.params.limit);

    data.result.preview = data.ast.html().trim();
  });
...
Parser.prototype.md2html = function (params, callback) {
  ...
  let text_length = getTextLength(ast);
  let data = {
    params,
    result: {
      html: '',
...
//
Parser.prototype.html2preview = function (params, callback) {
  let data = {
    params,
    ast: $('<div>').html(params.html),
    whitelist: [],
    self: this,
    result: {
      preview: ''
  ...
  if (!callback) {
    return this.bus.emit('html2preview', data).then(() => data.result);
  }

  this.bus.emit('html2preview', data, function (err) {
    if (err) {
      callback(err);
      return;
    }
...
// html2preview wrapper
//
function html2preview(params, callback) {
  return create(true).html2preview(params, callback);
}
