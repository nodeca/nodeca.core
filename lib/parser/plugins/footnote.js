...
module.exports = function () {
  ...
  return function (parser) {
    ...
    const footnote_ref_tpl = _.template(
      ...
      '<a href="#fn<%= n %>" id="fnref<%= n %><% if (sub) { print(":" + sub); } %>">[<%= n %>]</a>' +
      '</sup>'
    );
    const footnotes_list_tpl = _.template(
      ...
      '<% _.forEach(items, function (html, id) { %><li id="fn<%= Number(id) + 1 %>" class="footnote-item">' +
      '<%= html %>' +
      '</li>\n<% }); %>' +
      '</ol>\n' +
      '</section>'
    ...
    const back_ref_tpl = _.template(
      ' <a href="#fnref<%= n %><% if (sub) { print(":" + sub); } %>" class="footnote-backref">\u21a9</a>' /* ↩ */
    );


    parser.bus.on('md2html.render', function render_footnote(data) {
      ...
      data.ast.find('msg-footnote-data').each(function () {
        ...
        for (let i = 0; i < sub_count[id]; i++) {
          back_refs += back_ref_tpl({ n: Number(id) + 1, sub: i });
        }
      ...
      if (_.keys(items).length) {
        data.ast.append(footnotes_list_tpl({ items }));
      }
    });
