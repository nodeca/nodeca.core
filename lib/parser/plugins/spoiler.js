...
module.exports = function () {
  ...
  return function (parser) {
    ...
    var renderTpl = _.template(
      '<div class="spoiler">' +
        '<div class="spoiler__title">' +
          '<span class="spoiler__icon-collapse icon icon-collapse icon-space-after"></span>' +
          '<span class="spoiler__icon-expand icon icon-expand icon-space-after"></span>' +
          '<%= title %>' +
        '</div>' +
    ...
    parser.bus.on('render', function render_smiles(data) {
      data.ast.find('spoiler').each(function () {
        var element     = $(this);
        var replacement = $(renderTpl({
          title:   escape(element.attr('data-nd-title'))
        }));
        ...
        //
        replacement.children('.spoiler__content').append(element.children());
