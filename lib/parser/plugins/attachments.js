...
module.exports = function (N, mTypes) {
  ...
  return function (parser) {
    ...
    parser.md.renderer.rules.image = function (tokens, idx, options, env, _self) {
      var src = tokens[idx].attrs[tokens[idx].attrIndex('src')][1];

      var match = _.find(N.router.matchAll(src), function (match) {
        return match.meta.methods.get === 'users.media';
      });

      // It is not URL of media, continue
      if (!match) {
        return defaultImageRender(tokens, idx, options, env, _self);
      }

      return '<attach data-nd-media-id="' + match.params.media_id + '" data-nd-src="' + src + '"></attach>';
    };
    ...
    parser.bus.on('render', function render_attachments(data) {
      ...
      var binTpl = _.template('<a href="<%= href %>" class="thumb"><%= text %></a>');
      var imgTpl = _.template('<a href="<%= href %>" class="thumb"><img class="thumb__image" src="<%= src %>"></a>');
