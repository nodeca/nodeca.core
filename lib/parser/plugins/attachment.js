...
module.exports = function (N, pluginConfig) {
  ...
  return function (parser) {
    ...
    parser.bus.on('md2html.render', function render_attachments(data) {
    ...
    //
    parser.bus.on('html2preview.render', function replace_attach_to_icon(data) {
      data.whitelist.push('span.icon.icon-attach');

      data.ast.find('.attach').replaceWith('<span class="icon icon-attach"></span>');
    });
  };
};
