...
var beautify = require('./_lib/beautify_url');
var md_quote = require('./_lib/md-quote');
...
module.exports = function (N) {
  ...
  return function (parser) {
    var linkTpl = _.template('<a class="link link-ext" href="<%- href %>" target="_blank" rel="nofollow">' +
      '<%- content %></a>');
    var imageTpl = _.template('<a class="icon icon-picture ' +
      '<%- is_attach ? \'attach-collapsed\' : \'image-collapsed\' %>" ' +
      'href="<%- href %>"></a>');
    ...
    parser.bus.after('render', function quote_replace_contents(data) {
      data.ast.find('.quote__content').each(function () {
        ...
        quote.find('.image, .attach').each(function () {
          $(this).replaceWith(imageTpl({
            is_attach: $(this).hasClass('attach'),
