...
module.exports = function () {
  ...
  return function (parser) {
    ...
    parser.bus.on('md2html.render', function render_codeblock(data) {
      data.ast.find('msg-codeblock').each(function () {
        ...
        try {
          if (lang && lang !== 'auto' && parser.md.highlighter.getLanguage(lang)) {
            $el.replaceWith(tpl({
              lang,
              content: parser.md.highlighter.highlight(lang, content, true).value
            }));
            return;
          } else if (lang === 'auto') {
            let result = parser.md.highlighter.highlightAuto(content);
    ...
    // - code to text
    //
    parser.bus.on('html2preview.render', function replace_code(data) {
      data.whitelist.push('span.icon.icon-code');

      data.ast.find('pre.hljs').replaceWith('<span class="icon icon-code"></span>');
